...
use crate::{
  arithmetic::{eval_polynomial, kate_division, CurveAffine},
  poly::{
    commitment::{Blind, Params, ParamsProver, Prover},
    ipa::commitment::{self, IPACommitmentScheme, ParamsIPA},
    query::ProverQuery,
    Coeff, Polynomial,
  },
...
impl<'params, C: CurveAffine> Prover<'params, IPACommitmentScheme<C>> for ProverIPA<'params, C> {
  ...
  fn new(params: &'params ParamsIPA<C>) -> Self { Self { params } }
    ...
    R: RngCore,
  {
    ...
    let q_prime_poly = point_sets
      ...
      .fold(None, |q_prime_poly, (points, poly)| {
        let mut poly = points
          .iter()
          .fold(poly.clone().unwrap().values, |poly, point| kate_division(&poly, *point));
        poly.resize(self.params.n as usize, C::Scalar::ZERO);
        let poly = Polynomial { values: poly, _marker: PhantomData };
    ...
    let (p_poly, p_poly_blind) = q_polys.into_iter().zip(q_blinds).fold(
      ...
      |(q_prime_poly, q_prime_blind), (poly, blind)| {
        (q_prime_poly * *x_4 + &poly.unwrap(), Blind((q_prime_blind.0 * &(*x_4)) + &blind.0))
      },
    );
