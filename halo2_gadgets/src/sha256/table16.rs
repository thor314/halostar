...
impl<const LEN: usize> Bits<LEN> {
  fn spread<const SPREAD: usize>(&self) -> [bool; SPREAD] { spread_bits(self.0) }
}
...
impl<const LEN: usize> From<[bool; LEN]> for Bits<LEN> {
  fn from(bits: [bool; LEN]) -> Self { Self(bits) }
}

impl<const LEN: usize> From<&Bits<LEN>> for [bool; LEN] {
  fn from(bits: &Bits<LEN>) -> Self { bits.0 }
}
...
impl From<&Bits<16>> for u16 {
  fn from(bits: &Bits<16>) -> u16 { lebs2ip(&bits.0) as u16 }
}

impl From<u16> for Bits<16> {
  fn from(int: u16) -> Bits<16> { Bits(i2lebsp::<16>(int.into())) }
}

impl From<&Bits<32>> for u32 {
  fn from(bits: &Bits<32>) -> u32 { lebs2ip(&bits.0) as u32 }
}

impl From<u32> for Bits<32> {
  fn from(int: u32) -> Bits<32> { Bits(i2lebsp::<32>(int.into())) }
}
...
impl<const LEN: usize> AssignedBits<LEN> {
    ...
    <T as TryInto<[bool; LEN]>>::Error: std::fmt::Debug,
  {
    ...
    match column.column_type() {
      Any::Advice(_) =>
        region.assign_advice(annotation, column.try_into().unwrap(), offset, || value.clone()),
      Any::Fixed =>
        region.assign_fixed(annotation, column.try_into().unwrap(), offset, || value.clone()),
...
impl AssignedBits<16> {
  fn value_u16(&self) -> Value<u16> { self.value().map(|v| v.into()) }
...
impl AssignedBits<32> {
  fn value_u32(&self) -> Value<u32> { self.value().map(|v| v.into()) }
...
impl Table16Chip {
  ...
  pub fn configure(
    ...
    Table16Config { lookup, message_schedule, compression }
  }
...
impl Sha256Instructions<pallas::Base> for Table16Chip {
  ...
  fn initialization(
    ...
    self.config().compression.initialize_with_state(layouter, init_state.clone())
  }
  ...
  fn compress(
    ...
    config.compression.compress(layouter, initialized_state.clone(), w_halves)
  }
...
trait Table16Assignment {
  ...
  ) -> Result<((AssignedBits<16>, AssignedBits<16>), (AssignedBits<16>, AssignedBits<16>)), Error>
  {
    // Lookup R_0^{even}, R_0^{odd}, R_1^{even}, R_1^{odd}
    let r_0_even =
      SpreadVar::with_lookup(region, lookup, row - 1, r_0_even.map(SpreadWord::<16, 32>::new))?;
    let r_0_odd =
      SpreadVar::with_lookup(region, lookup, row, r_0_odd.map(SpreadWord::<16, 32>::new))?;
    let r_1_even =
      SpreadVar::with_lookup(region, lookup, row + 1, r_1_even.map(SpreadWord::<16, 32>::new))?;
    let r_1_odd =
      SpreadVar::with_lookup(region, lookup, row + 2, r_1_odd.map(SpreadWord::<16, 32>::new))?;

    // Assign and copy R_1^{odd}
    r_1_odd.spread.copy_advice(|| "Assign and copy R_1^{odd}", region, a_3, row)?;

    Ok(((r_0_even.dense, r_1_even.dense), (r_0_odd.dense, r_1_odd.dense)))
  }
  ...
  fn assign_sigma_outputs(
    ...
    let (even, _odd) =
      self.assign_spread_outputs(region, lookup, a_3, row, r_0_even, r_0_odd, r_1_even, r_1_odd)?;
...
mod tests {
  ...
  fn print_sha256_circuit() {
    ...
    let root = root.titled("16-bit Table SHA-256 Chip", ("sans-serif", 60)).unwrap();
