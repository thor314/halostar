        ...
        constraints: gate
          ...
          .map(|(i, constraint)| Constraint {
            ...
            expression: constraint.evaluate(
              ...
              &|query| {
                if query.phase == FirstPhase.to_sealed() {
                  ...
                  format!("A{}({})@{}", query.column_index, query.phase(), query.rotation.0)
                }
              },
            ...
            queries:    constraint.evaluate(
              ...
              &|query| {
                vec![format!("F{}@{}", query.column_index, query.rotation.0)].into_iter().collect()
              },
              &|query| {
              ...
              &|query| {
                vec![format!("I{}@{}", query.column_index, query.rotation.0)].into_iter().collect()
              },
              &|challenge| {
                vec![format!("C{}({})", challenge.index(), challenge.phase())].into_iter().collect()
              },
              &|a| a,
              &|mut a, mut b| {
                a.append(&mut b);
