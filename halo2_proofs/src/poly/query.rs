...
impl<'com, C: CurveAffine> PartialEq for PolynomialPointer<'com, C> {
  fn eq(&self, other: &Self) -> bool { std::ptr::eq(self.poly, other.poly) }
}
...
impl<'com, C: CurveAffine> Query<C::Scalar> for ProverQuery<'com, C> {
  ...
  fn get_point(&self) -> C::Scalar { self.point }

  fn get_eval(&self) -> Self::Eval { eval_polynomial(&self.poly[..], self.get_point()) }
...
impl<'com, C: CurveAffine, M: MSM<C>> Query<C::Scalar> for VerifierQuery<'com, C, M> {
  ...
  fn get_eval(&self) -> C::Scalar { self.eval }
