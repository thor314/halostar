...
#[cfg(test)]
impl CondSwapConfig {
  pub(crate) fn a(&self) -> Column<Advice> { self.a }
}
...
impl<F: PrimeField> CondSwapChip<F> {
  ...
  pub fn construct(config: CondSwapConfig) -> Self { CondSwapChip { config, _marker: PhantomData } }
}
...
mod tests {
  ...
  use super::{super::UtilitiesInstructions, CondSwapChip, CondSwapConfig, CondSwapInstructions};
  ...
  fn cond_swap() {
    ...
    impl<F: PrimeField> Circuit<F> for MyCircuit<F> {
      ...
      fn synthesize(
        ...
        let swapped_pair =
          chip.swap(layouter.namespace(|| "swap"), (a.clone(), self.b), self.swap)?;
