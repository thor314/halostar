...
impl<F: PrimeField> Gate<F> {
  ...
  pub fn two_bit_spread_and_range(
    ...
    let two_bit_spread = |dense: Expression<F>, spread: Expression<F>| {
      let (factor, lagrange_poly) =
        Self::lagrange_interpolate(dense, vec![0b00, 0b01, 0b10, 0b11], vec![
    ...
    std::iter::empty()
      .chain(Some(("two_bit_range_check", Self::range_check(dense.clone(), 0, (1 << 2) - 1))))
      .chain(Some(("two_bit_spread_check", two_bit_spread(dense, spread))))
  }
  ...
  pub fn three_bit_spread_and_range(
    ...
    let three_bit_spread = |dense: Expression<F>, spread: Expression<F>| {
      let (factor, lagrange_poly) = Self::lagrange_interpolate(
        ...
        vec![0b000000, 0b000001, 0b000100, 0b000101, 0b010000, 0b010001, 0b010100, 0b010101],
      );
    ...
    std::iter::empty()
      .chain(Some(("three_bit_range_check", Self::range_check(dense.clone(), 0, (1 << 3) - 1))))
      .chain(Some(("three_bit_spread_check", three_bit_spread(dense, spread))))
  }
}
