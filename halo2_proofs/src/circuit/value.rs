...
/// - It does not expose the enum cases, or provide an `Option::unwrap` equivalent. This helps to
///   ensure that unwitnessed values correctly propagate.
/// - It provides pass-through implementations of common traits such as `Add` and `Mul`, for
///   improved usability.
...
impl<V> Value<V> {
  ...
  pub const fn unknown() -> Self { Self { inner: None } }
  ...
  /// ```
  pub const fn known(value: V) -> Self { Self { inner: Some(value) } }

  /// Obtains the inner value for assigning into the circuit.
  ///
  /// Returns `Error::Synthesis` if this is [`Value::unknown()`].
  pub fn assign(self) -> Result<V, Error> { self.inner.ok_or(Error::Synthesis) }

  /// Converts from `&Value<V>` to `Value<&V>`.
  pub fn as_ref(&self) -> Value<&V> { Value { inner: self.inner.as_ref() } }

  /// Converts from `&mut Value<V>` to `Value<&mut V>`.
  pub fn as_mut(&mut self) -> Value<&mut V> { Value { inner: self.inner.as_mut() } }
  ...
  pub fn map<W, F: FnOnce(V) -> W>(self, f: F) -> Value<W> { Value { inner: self.inner.map(f) } }
...
impl<V: Neg> Neg for Value<V> {
  ...
  fn neg(self) -> Self::Output { Value { inner: self.inner.map(|v| -v) } }
}
...
where for<'v> V: Add<&'v V, Output = O>
{
  ...
  fn add(self, rhs: &Self) -> Self::Output { self + rhs.as_ref() }
}
...
where for<'v> &'v V: Add<V, Output = O>
{
  ...
  fn add(self, rhs: Value<V>) -> Self::Output { self.as_ref() + rhs }
}
...
where for<'v> V: Sub<&'v V, Output = O>
{
  ...
  fn sub(self, rhs: &Self) -> Self::Output { self - rhs.as_ref() }
}
...
where for<'v> &'v V: Sub<V, Output = O>
{
  ...
  fn sub(self, rhs: Value<V>) -> Self::Output { self.as_ref() - rhs }
}
...
where for<'v> V: Mul<&'v V, Output = O>
{
  ...
  fn mul(self, rhs: &Self) -> Self::Output { self * rhs.as_ref() }
}
...
where for<'v> &'v V: Mul<V, Output = O>
{
  ...
  fn mul(self, rhs: Value<V>) -> Self::Output { self.as_ref() * rhs }
}
...
impl<F: Field> From<Value<F>> for Value<Assigned<F>> {
  fn from(value: Value<F>) -> Self { Self { inner: value.inner.map(Assigned::from) } }
}
...
impl<F: Field> Add<F> for Value<Assigned<F>> {
  ...
  fn add(self, rhs: F) -> Self::Output { self + Value::known(rhs) }
}
...
impl<F: Field> Sub<F> for Value<Assigned<F>> {
  ...
  fn sub(self, rhs: F) -> Self::Output { self - Value::known(rhs) }
}
...
impl<F: Field> Mul<F> for Value<Assigned<F>> {
  ...
  fn mul(self, rhs: F) -> Self::Output { self * Value::known(rhs) }
}
...
impl<F: Field> Value<Assigned<F>> {
  ...
  pub fn evaluate(self) -> Value<F> { Value { inner: self.inner.map(|v| v.evaluate()) } }
}
