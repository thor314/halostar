...
impl<F: PrimeField> CompressionGate<F> {
  fn ones() -> Expression<F> { Expression::Constant(F::ONE) }
  ...
  > {
    let check_spread_and_range =
      Gate::three_bit_spread_and_range(c_lo.clone(), spread_c_lo.clone())
        .chain(Gate::three_bit_spread_and_range(c_mid.clone(), spread_c_mid.clone()))
        .chain(Gate::three_bit_spread_and_range(c_hi.clone(), spread_c_hi.clone()))
  ...
  > {
    let check_spread_and_range =
      Gate::three_bit_spread_and_range(a_lo.clone(), spread_a_lo.clone())
        ...
        .chain(Gate::three_bit_spread_and_range(b_hi.clone(), spread_b_hi.clone()))
        .chain(Gate::two_bit_spread_and_range(b_lo.clone(), spread_b_lo.clone()));
  ...
  > {
    let neg_check = {
      ...
      std::iter::empty().chain(Some(("lo_check", lo_check))).chain(Some(("hi_check", hi_check)))
    };
  ...
  pub fn s_digest(
    ...
    let check_lo_hi =
      |lo: Expression<F>, hi: Expression<F>, word: Expression<F>| lo + hi * F::from(1 << 16) - word;

    Constraints::with_selector(s_digest, [
