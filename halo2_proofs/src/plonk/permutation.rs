...
impl Argument {
  pub(crate) fn new() -> Self { Argument { columns: vec![] } }
  ...
  pub fn get_columns(&self) -> Vec<Column<Any>> { self.columns.clone() }
}
...
where C::Scalar: SerdePrimeField
{
  ...
  pub(super) fn read<R: io::Read>(reader: &mut R, format: SerdeFormat) -> io::Result<Self> {
    ...
    Ok(ProvingKey { permutations, polys, cosets })
  }

  /// Writes proving key for a single permutation argument to buffer using `Polynomial::write`.  
  pub(super) fn write<W: io::Write>(&self, writer: &mut W, format: SerdeFormat) -> io::Result<()> {
