...
impl<C: CurveAffine> Argument<C> {
  ...
  >(
    ...
    thread_seeds
      ...
      .for_each(|(mut rng, chunk)| chunk.iter_mut().for_each(|v| *v = C::Scalar::random(&mut rng)));
...
impl<C: CurveAffine> Committed<C> {
  ...
  >(
    ...
    let h_pieces = h_poly
      ...
      .collect::<Vec<_>>();
    drop(h_poly);
    let h_blinds: Vec<_> = h_pieces.iter().map(|_| Blind(C::Scalar::random(&mut rng))).collect();
...
impl<C: CurveAffine> Constructed<C> {
  pub(in crate::plonk) fn evaluate<E: EncodedChallenge<C>, T: TranscriptWrite<C, E>>(
    ...
    let h_poly = self.h_pieces.iter().rev().fold(domain.empty_coeff(), |acc, eval| acc * xn + eval);

    let h_blind =
      self.h_blinds.iter().rev().fold(Blind(C::Scalar::ZERO), |acc, eval| acc * Blind(xn) + *eval);
