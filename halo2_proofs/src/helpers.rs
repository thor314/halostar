...
pub enum SerdeFormat {
  ...
  /// When deserializing, checks are performed to ensure curve elements indeed lie on the curve and
  /// field elements are less than modulus.
  RawBytes,
    ...
    match format {
      SerdeFormat::Processed => {
        ...
        Option::from(Self::from_repr(compressed))
          .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "Invalid prime field point encoding"))
      },
      SerdeFormat::RawBytes => <Self as SerdeObject>::read_raw(reader),
      SerdeFormat::RawBytesUnchecked => Ok(<Self as SerdeObject>::read_raw_unchecked(reader)),
    }
...
pub(crate) fn read_polynomial_vec<R: io::Read, F: SerdePrimeField, B>(
  ...
  (0..len).map(|_| Polynomial::<F, B>::read(reader, format)).collect::<io::Result<Vec<_>>>()
}
...
/// Gets the total number of bytes of a slice of polynomials, assuming all polynomials are the same
/// length
