...
impl<Fixed: FixedPoints<pallas::Affine>> Config<Fixed> {
  pub(crate) fn configure(
    ...
    let config = Self { q_mul_fixed_short: meta.selector(), super_config };
  ...
  fn create_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    ...
    meta.create_gate("Short fixed-base mul gate", |meta| {
      ...
      Constraints::with_selector(q_mul_fixed_short, [
  ...
  where
    <Fixed as FixedPoints<pallas::Affine>>::ShortScalar: super::super::FixedPoint<pallas::Affine>,
  {
    let (scalar, acc, mul_b) = layouter.assign_region(
      ...
      |mut region| {
        ...
        let scalar = match scalar.running_sum {
          None =>
            self.decompose(&mut region, offset, (scalar.magnitude.clone(), scalar.sign.clone())),
          Some(_) => todo!("unimplemented for halo2_gadgets v0.1.0"),
        }?;

        let (acc, mul_b) = self.super_config.assign_region_inner::<_, NUM_WINDOWS_SHORT>(
          &mut region,
          offset,
          &(&scalar).into(),
          base,
    ...
    let result = layouter.assign_region(
      ...
      |mut region| {
        ...
        let sign =
          scalar.sign.copy_advice(|| "sign", &mut region, self.super_config.window, offset)?;
        ...
        let y_var =
          region.assign_advice(|| "y_var", self.super_config.add_config.y_p, offset, || y_val)?;

        Ok(EccPoint { x: magnitude_mul.x, y: y_var })
      },
    )?;
    ...
    // tested at the circuit-level.
    {
      ...
      scalar.magnitude.value().zip(scalar.sign.value()).zip(result.point()).assert_if_known(
...
pub mod tests {
  ...
  pub(crate) fn test_mul_fixed_short(
    ...
    fn load_magnitude_sign(
      ...
      let magnitude =
        chip.load_private(layouter.namespace(|| "magnitude"), column, Value::known(magnitude))?;
      let sign = chip.load_private(layouter.namespace(|| "sign"), column, Value::known(sign))?;
    ...
    let magnitude_signs = [
      ...
      ("-[2^64 - 1]B", pallas::Base::from(0xFFFF_FFFF_FFFF_FFFFu64), -pallas::Base::one()),
      // There is a single canonical sequence of window values for which a doubling occurs on the
      // last step: 1333333333333333333334 in octal.
      // [0xB6DB_6DB6_DB6D_B6DC] B
      ("mul_with_double", pallas::Base::from(0xB6DB_6DB6_DB6D_B6DCu64), pallas::Base::one()),
      (
    ...
    for (name, magnitude, sign) in magnitude_signs.iter() {
      let (result, _) = {
        let magnitude_sign =
          load_magnitude_sign(chip.clone(), layouter.namespace(|| *name), *magnitude, *sign)?;
      ...
      let scalar = {
        let magnitude = pallas::Scalar::from_repr(magnitude.to_repr()).unwrap();
        let sign =
          if *sign == pallas::Base::one() { pallas::Scalar::ONE } else { -pallas::Scalar::ONE };
        magnitude * sign
      };
      constrain_equal_non_id(chip.clone(), layouter.namespace(|| *name), base_val, scalar, result)?;
    }
  ...
  fn invalid_magnitude_sign() {
    ...
    impl Circuit<pallas::Base> for MyCircuit {
      ...
      fn synthesize(
        ...
        let magnitude_sign = {
          let magnitude =
            self.load_private(layouter.namespace(|| "load magnitude"), column, self.magnitude)?;
          ...
          assert_eq!(
            ...
            Err(vec![
              VerifyFailure::ConstraintNotSatisfied {
                constraint:  ((17, "Short fixed-base mul gate").into(), 0, "last_window_check",)
                  .into(),
    ...
    // Sign that is not +/- 1 should fail
    {
      ...
      assert_eq!(
        ...
        Err(vec![
          ...
          VerifyFailure::ConstraintNotSatisfied {
            constraint:  ((17, "Short fixed-base mul gate").into(), 3, "negation_check").into(),
            location:    FailureLocation::InRegion {
              region: (3, "Short fixed-base mul (most significant word)").into(),
              offset: 1,
            },
            cell_values: vec![
              (((Any::advice(), 1).into(), 0).into(), format_value(negation_check_y),),
              (((Any::advice(), 3).into(), 0).into(), format_value(negation_check_y),),
