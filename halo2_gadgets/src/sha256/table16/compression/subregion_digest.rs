...
impl CompressionConfig {
  ...
  pub fn assign_digest(
    ...
    a.dense_halves.0.copy_advice(|| "a_lo", region, a_3, abcd_row)?;
    a.dense_halves.1.copy_advice(|| "a_hi", region, a_4, abcd_row)?;
    let a = a.dense_halves.value();
    region.assign_advice(|| "a", a_5, abcd_row, || a.map(|a| pallas::Base::from(a as u64)))?;
    ...
    e.dense_halves.0.copy_advice(|| "e_lo", region, a_3, efgh_row)?;
    e.dense_halves.1.copy_advice(|| "e_hi", region, a_4, efgh_row)?;
    let e = e.dense_halves.value();
    region.assign_advice(|| "e", a_5, efgh_row, || e.map(|e| pallas::Base::from(e as u64)))?;
