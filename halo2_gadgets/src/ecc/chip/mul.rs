...
use halo2curves::{group::ff::PrimeField, pasta::pallas};
...
impl Config {
  ...
  fn create_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    ...
    meta.create_gate("LSB check", |meta| {
      ...
      Constraints::with_selector(q_mul_lsb, [
  ...
  pub(super) fn assign(
    ...
    let (result, zs): (EccPoint, Vec<Z<pallas::Base>>) = layouter.assign_region(
      ...
      |mut region| {
        ...
        let acc = self.add_config.assign_region(&base_point, &base_point, offset, &mut region)?;
        ...
        // Check that the correct multiple is obtained.
        {
          ...
          let base = base.point();
          let alpha =
            alpha.value().map(|alpha| pallas::Scalar::from_repr(alpha.to_repr()).unwrap());
  ...
  /// - `acc_x` and `acc_y` are assigned in row `offset` by the previous complete addition. They
  ///   will be copied into themselves.
  /// - `z_1 is assigned in row `offset` by the mul::complete region assignment. We only use its
  ///   value here.
  ///
  ...
  fn process_lsb(
    ...
    let z_0 = {
      ...
      let z_0_cell =
        region.assign_advice(|| "z_0", self.complete_config.z_complete, offset + 1, || z_0_val)?;

      Z(z_0_cell)
    };

    // Copy in `base_x`, `base_y` to use in the LSB gate
    base.x().copy_advice(|| "copy base_x", region, self.add_config.x_p, offset + 1)?;
    base.y().copy_advice(|| "copy base_y", region, self.add_config.y_p, offset + 1)?;

    // If `lsb` is 0, return `Acc + (-P)`. If `lsb` is 1, simply return `Acc + 0`.
    let x =
      lsb.and_then(|lsb| if !lsb { base.x.value().cloned() } else { Value::known(Assigned::Zero) });

    let y = lsb.and_then(|lsb| if !lsb { -base.y.value() } else { Value::known(Assigned::Zero) });
    ...
    let p = EccPoint { x: x_cell, y: y_cell };
...
fn decompose_for_scalar_mul(scalar: Value<&pallas::Base>) -> Vec<Value<bool>> {
  ...
  let bitstring = scalar.map(|scalar| {
    ...
    let bitstring = {
      ...
      le_bytes.into_iter().flat_map(|byte| (0..8).map(move |shift| (byte >> shift) % 2 == 1))
    };

    // Take the first 255 bits.
    bitstring.take(pallas::Scalar::NUM_BITS as usize).collect::<Vec<_>>()
  });
...
pub mod tests {
  ...
  pub(crate) fn test_mul(
    ...
    // uses complete addition for the final bits of the scalar.
    {
      ...
      let (result, _) = {
        let scalar =
          chip.load_private(layouter.namespace(|| "zero"), column, Value::known(scalar_val))?;
        let scalar = ScalarVar::from_base(
          chip.clone(),
          layouter.namespace(|| "ScalarVar from_base"),
          &scalar,
    ...
    // [-1]B (the largest possible base field element)
    {
      ...
      let (result, _) = {
        let scalar =
          chip.load_private(layouter.namespace(|| "-1"), column, Value::known(scalar_val))?;
        let scalar = ScalarVar::from_base(
          chip.clone(),
          layouter.namespace(|| "ScalarVar from_base"),
          &scalar,
      ...
      constrain_equal_non_id(chip, layouter.namespace(|| "[-1]B"), p_val, scalar_val, result)?;
    }
