...
use crate::{
  ...
  sinsemilla::{
    ...
    primitives as sinsemilla, CommitDomains, HashDomains, MessagePiece, SinsemillaInstructions,
  },
  utilities::{
    cond_swap::{CondSwapChip, CondSwapConfig, CondSwapInstructions},
    RangeConstrained, UtilitiesInstructions,
  },
};
  ...
  Commit: CommitDomains<pallas::Affine, F, Hash>,
{
  ...
  pub fn configure(
    ...
    MerkleConfig { advices, q_decompose, cond_swap_config, sinsemilla_config }
  }

  /// Constructs a [`MerkleChip`] given a [`MerkleConfig`].
  pub fn construct(config: MerkleConfig<Hash, Commit, F>) -> Self { MerkleChip { config } }
}
  ...
  Commit: CommitDomains<pallas::Affine, F, Hash> + Eq,
{
  ...
  fn hash_layer(
    ...
    // c = bits 5..=254 of `right`
    let c = MessagePiece::from_subpieces(self.clone(), layouter.namespace(|| "Witness c"), [
      RangeConstrained::bitrange_of(right.value(), 5..(pallas::Base::NUM_BITS as usize)),
    ])?;
    ...
    // |  z1_a |  z1_b |  b_1  |  b_2  |   l   |      0      |
    {
      layouter.assign_region(
        ...
        |mut region| {
          ...
          a.inner().cell_value().copy_advice(|| "copy a", &mut region, config.advices[0], 0)?;
          // Copy and assign `b` at the correct position.
          b.inner().cell_value().copy_advice(|| "copy b", &mut region, config.advices[1], 0)?;
          // Copy and assign `c` at the correct position.
          c.inner().cell_value().copy_advice(|| "copy c", &mut region, config.advices[2], 0)?;
          ...
          b_1.inner().copy_advice(|| "b_1", &mut region, config.advices[2], 1)?;
          // Copy `b_2`, which has been constrained to be a 5-bit value
          b_2.inner().copy_advice(|| "b_2", &mut region, config.advices[3], 1)?;
    ...
    #[cfg(test)]
    {
      ...
      left.value().zip(right.value()).zip(hash.value()).assert_if_known(|((left, right), hash)| {
        let l = i2lebsp::<10>(l as u64);
        let left: Vec<_> =
          left.to_le_bits().iter().by_vals().take(pallas::Base::NUM_BITS as usize).collect();
        let right: Vec<_> =
          right.to_le_bits().iter().by_vals().take(pallas::Base::NUM_BITS as usize).collect();
