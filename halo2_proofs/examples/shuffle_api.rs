...
impl<F: Field> ShuffleChip<F> {
  fn construct(config: ShuffleConfig) -> Self { Self { config, _marker: PhantomData } }
  ...
  fn configure(
    ...
    ShuffleConfig { input_0, input_1, shuffle_0, shuffle_1, s_input, s_shuffle }
  }
}
...
impl<F: Field> Circuit<F> for MyCircuit<F> {
  ...
  fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
    ...
    layouter.assign_region(
      || "load inputs",
      |mut region| {
        for (i, (input_0, input_1)) in self.input_0.iter().zip(self.input_1.iter()).enumerate() {
          region.assign_advice(|| "input_0", ch.config.input_0, i, || *input_0)?;
          region.assign_fixed(|| "input_1", ch.config.input_1, i, || Value::known(*input_1))?;
...
fn main() {
  ...
  const K: u32 = 4;
  let input_0 = [1, 2, 4, 1].map(|e: u64| Value::known(Fp::from(e))).to_vec();
  let input_1 = [10, 20, 40, 10].map(Fp::from).to_vec();
  let shuffle_0 = [4, 1, 1, 2].map(|e: u64| Value::known(Fp::from(e))).to_vec();
  let shuffle_1 = [40, 10, 10, 20].map(|e: u64| Value::known(Fp::from(e))).to_vec();
  let circuit = MyCircuit { input_0, input_1, shuffle_0, shuffle_1 };
