...
use crate::{
  ...
  poly::{
    commitment::{Blind, Params, ParamsProver, Prover},
    kzg::commitment::{KZGCommitmentScheme, ParamsKZG},
    query::{PolynomialPointer, ProverQuery},
...
fn div_by_vanishing<F: Field>(poly: Polynomial<F, Coeff>, roots: &[F]) -> Vec<F> {
  let poly = roots.iter().fold(poly.values, |poly, point| kate_division(&poly, *point));
...
impl<'a, C: CurveAffine> Commitment<C::Scalar, PolynomialPointer<'a, C>> {
  fn extend(&self, points: &[C::Scalar]) -> CommitmentExtension<'a, C> {
    ...
    let low_degree_equivalent = Polynomial { values: poly, _marker: PhantomData };
...
impl<'a, C: CurveAffine> CommitmentExtension<'a, C> {
  ...
  fn quotient_contribution(&self) -> Polynomial<C::Scalar, Coeff> {
    ...
    parallelize(&mut p_x.values[0..len], |lhs, start| {
      for (lhs, rhs) in lhs.iter_mut().zip(self.low_degree_equivalent.values[start..].iter()) {
        *lhs -= *rhs;
      }
    });
    p_x
...
impl<'a, E: Engine> ProverSHPLONK<'a, E> {
  ...
  pub fn new(params: &'a ParamsKZG<E>) -> Self { Self { params } }
}
  ...
  E::G2Affine: SerdeCurveAffine,
{
  ...
  fn new(params: &'params ParamsKZG<E>) -> Self { Self { params } }
    ...
    R: RngCore,
  {
    ...
    let quotient_contribution =
      |rotation_set: &RotationSetExtension<E::G1Affine>| -> Polynomial<E::Scalar, Coeff> {
        ...
        Polynomial { values: poly, _marker: PhantomData }
      };
    ...
    let quotient_polynomials =
      rotation_sets.par_iter().map(quotient_contribution).collect::<Vec<_>>();
    ...
    let h_x = Polynomial { values: h_x, _marker: PhantomData };
