...
impl Config {
  pub(super) fn configure(
    ...
    let config = Self { q_mul_decompose_var: meta.selector(), z_complete, add_config };
  ...
  fn create_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    ...
    meta.create_gate("Decompose scalar for complete bits of variable-base mul", |meta| {
      ...
      Constraints::with_selector(q_mul_decompose_var, [
  ...
  pub(super) fn assign_region(
    ...
    for (iter, k) in bits.iter().enumerate() {
      ...
      z = {
        ...
        let z_val =
          z.value() * Value::known(pallas::Base::from(2)) + k.map(|k| pallas::Base::from(k as u64));
        let z_cell = region.assign_advice(|| "z", self.z_complete, row + offset + 2, || z_val)?;
        Z(z_cell)
      };
      zs.push(z.clone());
      ...
      let y_p = {
        let base_y =
          base.y.copy_advice(|| "Copy `base.y`", region, self.z_complete, row + offset + 1)?;
      ...
      let U = EccPoint { x: base.x.clone(), y: y_p };

      // Acc + U
      let tmp_acc = self.add_config.assign_region(&U, &acc, row + offset, region)?;

      // Acc + U + Acc
      acc = self.add_config.assign_region(&acc, &tmp_acc, row + offset + 1, region)?;
    }
    Ok((acc, zs))
  }
}
