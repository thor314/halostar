  ...
  for SingleChipLayouter<'a, F, CS>
{
  ...
  fn get_challenge(&self, challenge: Challenge) -> Value<F> { self.cs.get_challenge(challenge) }
...
///
/// - The outer `Option` tracks whether the value in row 0 of the table column has been assigned
///   yet. This will always be `Some` once a valid table has been completely assigned.
/// - The inner `Value` tracks whether the underlying `Assignment` is evaluating witnesses or not.
