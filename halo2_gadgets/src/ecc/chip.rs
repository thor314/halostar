...
impl EccPoint {
  ...
  pub fn x(&self) -> AssignedCell<pallas::Base, pallas::Base> { self.x.clone().evaluate() }

  /// The cell containing the affine short-Weierstrass y-coordinate,
  /// or 0 for the zero point.
  pub fn y(&self) -> AssignedCell<pallas::Base, pallas::Base> { self.y.clone().evaluate() }

  #[cfg(test)]
  fn is_identity(&self) -> Value<bool> { self.x.value().map(|x| x.is_zero_vartime()) }
}
...
impl From<NonIdentityEccPoint> for EccPoint {
  fn from(non_id_point: NonIdentityEccPoint) -> Self {
    Self { x: non_id_point.x, y: non_id_point.y }
  }
}
...
impl<FixedPoints: super::FixedPoints<pallas::Affine>> EccChip<FixedPoints> {
  ...
  pub fn construct(config: <Self as Chip<pallas::Base>>::Config) -> Self { Self { config } }
    ...
    FixedPoint<pallas::Affine, FixedScalarKind = ShortScalar>,
{
  ...
  fn witness_point(
    ...
    layouter.assign_region(|| "witness point", |mut region| config.point(value, 0, &mut region))
  }
  ...
  fn add<A: Into<Self::Point> + Clone, B: Into<Self::Point> + Clone>(
    ...
    layouter.assign_region(
      ...
      |mut region| config.assign_region(&(a.clone()).into(), &(b.clone()).into(), 0, &mut region),
    )
  }
  ...
  fn mul(
    ...
    match scalar {
      ScalarVar::BaseFieldElem(scalar) =>
        config.assign(layouter.namespace(|| "variable-base scalar mul"), scalar.clone(), base),
      ScalarVar::FullWidth => {
        todo!()
      },
    }
  ...
  fn mul_fixed(
    ...
    config.assign(layouter.namespace(|| format!("fixed-base mul of {:?}", base)), scalar, base)
  }
