...
impl Config {
  pub(super) fn configure(
    ...
    let config = Self { q_mul_overflow: meta.selector(), lookup_config, advices };
  ...
  fn create_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    ...
    meta.create_gate("overflow checks", |meta| {
      ...
      let two_pow_130 = two_pow_124.clone() * Expression::Constant(pallas::Base::from_u128(1 << 6));
  ...
  pub(super) fn overflow_check(
    ...
    let s = {
      ...
      layouter.assign_region(
        || "s = alpha + k_254 ⋅ 2^130",
        |mut region| {
          region.assign_advice(|| "s = alpha + k_254 ⋅ 2^130", self.advices[0], 0, || s_val)
        },
      )?
    };
    ...
    layouter.assign_region(
      ...
      |mut region| {
        ...
        // Witness η = inv0(z_130), where inv0(x) = 0 if x = 0, 1/x otherwise
        {
          ...
          region.assign_advice(|| "η = inv0(z_130)", self.advices[0], offset + 2, || eta)?;
        }
        ...
        // Copy original alpha
        alpha.copy_advice(|| "copy original alpha", &mut region, self.advices[1], offset + 1)?;
