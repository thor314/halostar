...
impl<F: PrimeFieldBits, const WINDOW_NUM_BITS: usize> RunningSumConfig<F, WINDOW_NUM_BITS> {
  ...
  pub(crate) fn q_range_check(&self) -> Selector { self.q_range_check }
  ...
  pub fn configure(
    ...
    let config = Self { q_range_check, z, _marker: PhantomData };
  ...
  fn decompose(
    ...
    for (i, word) in words.iter().enumerate() {
      ...
      let z_next = {
        ...
        region.assign_advice(|| format!("z_{:?}", i + 1), self.z, offset + i + 1, || z_next_val)?
      };
...
mod tests {
  ...
  fn test_running_sum() {
      ...
      > Circuit<F> for MyCircuit<F, WORD_NUM_BITS, WINDOW_NUM_BITS, NUM_WINDOWS>
    {
      ...
      fn without_witnesses(&self) -> Self { Self { alpha: Value::unknown(), strict: self.strict } }
    ...
    // Random base field element
    {
      ...
      let circuit: MyCircuit<pallas::Base, L_BASE, FIXED_BASE_WINDOW_SIZE, { NUM_WINDOWS }> =
        MyCircuit { alpha: Value::known(alpha), strict: true };
    ...
    // Random 64-bit word
    {
      ...
      // Strict full decomposition should pass.
      let circuit: MyCircuit<pallas::Base, L_SHORT, FIXED_BASE_WINDOW_SIZE, { NUM_WINDOWS_SHORT }> =
        MyCircuit { alpha: Value::known(alpha), strict: true };
      let prover = MockProver::<pallas::Base>::run(8, &circuit, vec![]).unwrap();
      assert_eq!(prover.verify(), Ok(()));
    }
    ...
    // 2^66
    {
      ...
      assert_eq!(
        ...
        Err(vec![
          ...
          VerifyFailure::Permutation {
            ...
            location: FailureLocation::InRegion { region: (0, "decompose").into(), offset: 22 },
          },
          VerifyFailure::Permutation {
            column:   (Any::advice(), 0).into(),
            location: FailureLocation::InRegion { region: (0, "decompose").into(), offset: 45 },
          },
        ])
      );
      ...
      let circuit: MyCircuit<
        ...
        { NUM_WINDOWS_SHORT },
      > = MyCircuit { alpha: Value::known(alpha), strict: false };
      let prover = MockProver::<pallas::Base>::run(8, &circuit, vec![]).unwrap();
      assert_eq!(prover.verify(), Ok(()));
    }
  }
