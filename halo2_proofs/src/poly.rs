...
use crate::{arithmetic::parallelize, helpers::SerdePrimeField, plonk::Assigned, SerdeFormat};
...
impl<F, B> Index<usize> for Polynomial<F, B> {
  ...
  fn index(&self, index: usize) -> &F { self.values.index(index) }
}

impl<F, B> IndexMut<usize> for Polynomial<F, B> {
  fn index_mut(&mut self, index: usize) -> &mut F { self.values.index_mut(index) }
}

impl<F, B> Index<RangeFrom<usize>> for Polynomial<F, B> {
  type Output = [F];

  fn index(&self, index: RangeFrom<usize>) -> &[F] { self.values.index(index) }
}

impl<F, B> IndexMut<RangeFrom<usize>> for Polynomial<F, B> {
  fn index_mut(&mut self, index: RangeFrom<usize>) -> &mut [F] { self.values.index_mut(index) }
}

impl<F, B> Index<RangeFull> for Polynomial<F, B> {
  type Output = [F];

  fn index(&self, index: RangeFull) -> &[F] { self.values.index(index) }
}

impl<F, B> IndexMut<RangeFull> for Polynomial<F, B> {
  fn index_mut(&mut self, index: RangeFull) -> &mut [F] { self.values.index_mut(index) }
}

impl<F, B> Deref for Polynomial<F, B> {
  type Target = [F];

  fn deref(&self) -> &[F] { &self.values[..] }
}

impl<F, B> DerefMut for Polynomial<F, B> {
  fn deref_mut(&mut self) -> &mut [F] { &mut self.values[..] }
}
...
impl<F: SerdePrimeField, B> Polynomial<F, B> {
  ...
  pub(crate) fn read<R: io::Read>(reader: &mut R, format: SerdeFormat) -> io::Result<Self> {
    ...
    (0..poly_len)
      ...
      .map(|values| Self { values, _marker: PhantomData })
  }
...
impl<F: Field> Polynomial<F, LagrangeCoeff> {
  ...
  pub fn rotate(&self, rotation: Rotation) -> Polynomial<F, LagrangeCoeff> {
    ...
    Polynomial { values, _marker: PhantomData }
  }
}
...
impl<F: Field, B: Basis> Mul<F> for Polynomial<F, B> {
  ...
  fn mul(mut self, rhs: F) -> Polynomial<F, B> {
    if rhs == F::ZERO {
      return Polynomial { values: vec![F::ZERO; self.len()], _marker: PhantomData };
    }
    if rhs == F::ONE {
      return self;
    }
...
impl Rotation {
  ...
  pub fn cur() -> Rotation { Rotation(0) }

  /// The previous location in the evaluation domain
  pub fn prev() -> Rotation { Rotation(-1) }

  /// The next location in the evaluation domain
  pub fn next() -> Rotation { Rotation(1) }
}
