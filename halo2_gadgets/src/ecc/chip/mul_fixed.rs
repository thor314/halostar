...
use halo2_proofs::{
  ...
  plonk::{
    Advice, Column, ConstraintSystem, Constraints, Error, Expression, Fixed, Selector, VirtualCells,
  },
  poly::Rotation,
};
...
impl<FixedPoints: super::FixedPoints<pallas::Affine>> Config<FixedPoints> {
  ...
  pub(super) fn configure(
    ...
    for advice in [config.window, config.u].iter() {
      assert_ne!(*advice, config.add_config.x_qr, "Do not overlap with output columns of add.");
      assert_ne!(*advice, config.add_config.y_qr, "Do not overlap with output columns of add.");
    }
  ...
  fn running_sum_coords_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    meta.create_gate("Running sum coordinates check", |meta| {
      let q_mul_fixed_running_sum = meta.query_selector(self.running_sum_config.q_range_check());
  ...
  fn coords_check(
    ...
    let window_pow: Vec<Expression<pallas::Base>> = (0..H)
      .map(|pow| {
        (0..pow).fold(Expression::Constant(pallas::Base::one()), |acc, _| acc * window.clone())
      })
      .collect();
    ...
    vec![("check x", x_check), ("check y", y_check), ("on-curve", on_curve)]
  }
  ...
  fn assign_fixed_constants<F: FixedPoint<pallas::Affine>, const NUM_WINDOWS: usize>(
    ...
    for window in 0..NUM_WINDOWS {
      ...
      for k in 0..H {
        region.assign_fixed(
          || format!("Lagrange interpolation coeff for window: {:?}, k: {:?}", window, k),
  ...
  fn add_incomplete<F: FixedPoint<pallas::Affine>, const NUM_WINDOWS: usize>(
            ...
            .skip(1)
    {
      ...
      let mul_b = self.process_lower_bits::<_, NUM_WINDOWS>(region, offset, w, k, k_usize, base)?;
      ...
      acc = self.add_incomplete_config.assign_region(&mul_b, &acc, offset + w, region)?;
    }
    Ok(acc)
  }
  ...
  fn process_msb<F: FixedPoint<pallas::Affine>, const NUM_WINDOWS: usize>(
    ...
    self.process_window::<_, NUM_WINDOWS>(region, offset, NUM_WINDOWS - 1, k_usize, scalar, base)
  }
}
...
impl From<&EccScalarFixed> for ScalarFixed {
  fn from(scalar_fixed: &EccScalarFixed) -> Self { Self::FullWidth(scalar_fixed.clone()) }
}

impl From<&EccScalarFixedShort> for ScalarFixed {
  fn from(scalar_fixed: &EccScalarFixedShort) -> Self { Self::Short(scalar_fixed.clone()) }
}
  ...
  fn windows_field(&self) -> Vec<Value<pallas::Scalar>> {
    ...
    match self {
      ...
      Self::Short(scalar) => running_sum_to_windows(
        scalar.running_sum.as_ref().expect("EccScalarFixedShort has been constrained").to_vec(),
      ),
      Self::FullWidth(scalar) => scalar
        ...
        .map(|bits| {
          ...
          // and pallas::Scalar are the same. They happen to be, but we need to
          // be careful if this is generalised.
          bits.value().map(|value| pallas::Scalar::from_repr(value.to_repr()).unwrap())
        })
        .collect::<Vec<_>>(),
    }
  }
