...
use crate::{
  ...
  plonk::{
    ...
    Advice, Any, Assigned, Assignment, Challenge, Circuit, Column, ColumnType, ConstraintSystem,
    Error, Expression, FirstPhase, Fixed, FloorPlanner, Instance, Phase, Selector, VirtualCell,
  },
  poly::Rotation,
};
...
#[cfg(feature = "dev-graph")] mod graph;
...
impl<F: Field> Mul for Value<F> {
  ...
  fn mul(self, rhs: Self) -> Self::Output {
    match (self, rhs) {
      ...
      (Value::Real(x), Value::Poison) | (Value::Poison, Value::Real(x)) if x.is_zero_vartime() =>
        Value::Real(F::ZERO),
      _ => Value::Poison,
    }
  }
...
/// let circuit = MyCircuit { a: Value::known(2), b: Value::known(4) };
///
...
///     location:    FailureLocation::InRegion { region: (0, "Example region").into(), offset: 0 },
///     cell_values: vec![
...
/// assert_eq!(result.unwrap_err().downcast_ref::<String>().unwrap(), "n=4, minimum_rows=8, k=2");
/// ```
#[derive(Debug)]
...
impl<F: Field> MockProver<F> {
  fn in_phase<P: Phase>(&self, phase: P) -> bool { self.current_phase == phase.to_sealed() }
}
...
impl<F: Field> Assignment<F> for MockProver<F> {
  ...
  where
    ...
    self.current_region.as_mut().unwrap().enabled_selectors.entry(*selector).or_default().push(row);
    ...
    AR: Into<String>,
  {
    ...
    match to().into_field().evaluate().assign() {
      Ok(to) => {
        let value =
          self.advice.get_mut(column.index()).and_then(|v| v.get_mut(row)).expect("bounds failure");
...
impl<F: FromUniformBytes<64> + Ord> MockProver<F> {
  ...
  pub fn verify_at_rows<I: Clone + Iterator<Item = usize>>(
    ...
    let load = |expression: &Expression<F>, row| {
      expression.evaluate_lazy(
        ...
        &|query| {
          ...
          self.fixed[column_index][(row as i32 + n + rotation) as usize % n as usize].into()
        },
        &|query| {
          let query = self.cs.advice_queries[query.index.unwrap()];
          let column_index = query.0.index();
          let rotation = query.1 .0;
          self.advice[column_index][(row as i32 + n + rotation) as usize % n as usize].into()
        },
        &|query| {
          ...
          let rotation = query.1 .0;
          Value::Real(
            self.instance[column_index][(row as i32 + n + rotation) as usize % n as usize],
          )
        },
        &|challenge| Value::Real(self.challenges[challenge.index()]),
        &|a| -a,
    ...
    let lookup_errors = self.cs.lookups.iter().enumerate().flat_map(|(lookup_index, lookup)| {
      ...
      // match the fill row.
      let fill_row: Vec<_> =
        lookup.table_expressions.iter().map(move |c| load(c, self.usable_rows.end - 1)).collect();
      ...
      if table_identifier != cached_table_identifier {
        ...
        cached_table = self
          ...
          .filter_map(|table_row| {
            let t = lookup.table_expressions.iter().map(move |c| load(c, table_row)).collect();
      ...
      let mut inputs: Vec<(Vec<_>, usize)> = lookup_input_row_ids
        ...
        .filter_map(|input_row| {
          let t = lookup.input_expressions.iter().map(move |c| load(c, input_row)).collect();
    ...
    let shuffle_errors =
      self.cs.shuffles.iter().enumerate().flat_map(|(shuffle_index, shuffle)| {
        ...
        let mut shuffle_rows: Vec<Vec<Value<F>>> = self
          ...
          .clone()
          .map(|row| {
            let t = shuffle.shuffle_expressions.iter().map(move |c| load(c, row)).collect();
            t
          })
          .collect();
        shuffle_rows.sort();
        ...
        let mut input_rows: Vec<(Vec<Value<F>>, usize)> = self
          ...
          .clone()
          .map(|input_row| {
            let t = shuffle.input_expressions.iter().map(move |c| load(c, input_row)).collect();
  ...
  /// Returns `Ok(())` if this `MockProver` is satisfied, or a list of errors indicating
  /// the reasons that the circuit is not satisfied.
  /// Constraints are only checked at `gate_row_ids`, and lookup inputs are only checked at
  /// `lookup_input_row_ids`, parallelly.
  pub fn verify_at_rows_par<I: Clone + Iterator<Item = usize>>(
    ...
    let load = |expression: &Expression<F>, row| {
      expression.evaluate_lazy(
        ...
        &|_| panic!("virtual selectors are removed during optimization"),
        &|query| {
          self.fixed[query.column_index][(row as i32 + n + query.rotation.0) as usize % n as usize]
            .into()
        },
        &|query| {
          self.advice[query.column_index][(row as i32 + n + query.rotation.0) as usize % n as usize]
            .into()
        },
        &|query| {
          Value::Real(
  ...
  /// Returns the list of Fixed Columns used within a MockProver instance and the associated values
  /// contained on each Cell.
  pub fn fixed(&self) -> &Vec<Vec<CellValue<F>>> { &self.fixed }
  ...
  fn unassigned_cell() {
    ...
    assert_eq!(
      ...
      Err(vec![VerifyFailure::CellNotAssigned {
        ...
        gate_offset: 1,
        column:      Column::new(1, Any::Advice(Advice { phase: FirstPhase.to_sealed() })),
        offset:      1,
      }])
    );
  }
      ...
      fn synthesize(
        ...
        layouter.assign_region(
          ...
          |mut region| {
            ...
            // Assign a = 2 and a = 6.
            region.assign_advice(|| "a = 2", config.a, 0, || Value::known(Fp::from(2)))?;
            region.assign_advice(|| "a = 6", config.a, 1, || Value::known(Fp::from(6)))?;
        ...
        layouter.assign_region(
          ...
          |mut region| {
            ...
            // Assign a = 4.
            region.assign_advice(|| "a = 4", config.a, 0, || Value::known(Fp::from(4)))?;

            // BUG: Assign a = 5, which doesn't exist in the table!
            region.assign_advice(|| "a = 5", config.a, 1, || Value::known(Fp::from(5)))?;
  ...
  fn contraint_unsatisfied() {
      ...
      fn synthesize(
        ...
        layouter.assign_region(
          ...
          |mut region| {
            ...
            // Assign c = 5.
            region.assign_advice(|| "c", config.c, 0, || Value::known(Fp::from(5u64)))?;
            // Assign d = 7.
            region.assign_fixed(|| "d", config.d, 0, || Value::known(Fp::from(7u64)))?;
            Ok(())
          },
        )?;
        layouter.assign_region(
          ...
          |mut region| {
            ...
            // Note that none of the terms cancel eachother. Therefore we will have a constraint
            // that is non satisfied for the `Equalty check` gate.
            Ok(())
          },
        )
      }
    ...
    assert_eq!(
      ...
      Err(vec![VerifyFailure::ConstraintNotSatisfied {
        ...
        location:    FailureLocation::InRegion { region: (1, "Wrong synthesis").into(), offset: 0 },
        cell_values: vec![
          (
            ((Any::Advice(Advice { phase: FirstPhase.to_sealed() }), 0).into(), 0).into(),
            "1".to_string()
          ),
          (
            ((Any::Advice(Advice { phase: FirstPhase.to_sealed() }), 1).into(), 0).into(),
            "0".to_string()
          ),
          (
            ((Any::Advice(Advice { phase: FirstPhase.to_sealed() }), 2).into(), 0).into(),
            "0x5".to_string()
          ),
