...
impl<E: Engine> MSMKZG<E> {
  ...
  pub fn new() -> Self { MSMKZG { scalars: vec![], bases: vec![] } }
...
impl<E: Engine + Debug> MSM<E::G1Affine> for MSMKZG<E> {
  ...
  fn check(&self) -> bool { bool::from(self.eval().is_identity()) }
  ...
  fn bases(&self) -> Vec<E::G1> { self.bases.clone() }

  fn scalars(&self) -> Vec<E::Scalar> { self.scalars.clone() }
}
...
impl<E: Engine + Debug> PreMSM<E> {
  pub(crate) fn new() -> Self { PreMSM { projectives_msms: vec![] } }
  ...
  pub(crate) fn add_msm(&mut self, other: MSMKZG<E>) { self.projectives_msms.push(other); }
}

impl<'params, E: MultiMillerLoop + Debug> From<&'params ParamsKZG<E>> for DualMSM<'params, E> {
  fn from(params: &'params ParamsKZG<E>) -> Self { DualMSM::new(params) }
}
...
impl<'a, E: MultiMillerLoop + Debug> DualMSM<'a, E> {
  ...
  pub fn check(self) -> bool {
    ...
    let (term_1, term_2) = ((&left.into(), &s_g2_prepared), (&right.into(), &n_g2_prepared));
