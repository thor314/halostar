...
impl Ord for AllocatedRegion {
  fn cmp(&self, other: &Self) -> cmp::Ordering { self.start.cmp(&other.start) }
}
...
impl EmptySpace {
  pub(crate) fn range(&self) -> Option<Range<usize>> { self.end.map(|end| self.start..end) }
}
        ...
        Some(if let Some(region) = region {
          if end.map(|end| region.start >= end).unwrap_or(false) {
            ...
            let ret = if *row < region.start {
              Some(EmptySpace { start: *row, end: Some(region.start) })
            } else {
              None
            };
...
/// - `slack` is the maximum number of rows the start could be moved down, taking into account prior
///   columns.
fn first_fit_region(
  ...
  for space in column_allocations.entry(*c).or_default().clone().free_intervals(start, end) {
    ...
    let s_slack =
      space.end.map(|end| (end as isize - space.start as isize) - region_length as isize);
    ...
    if s_slack.unwrap_or(0) >= 0 {
      ...
      if let Some(row) = row {
        ...
        column_allocations
          ...
          .unwrap()
          .0
          .insert(AllocatedRegion { start: row, length: region_length });
        return Some(row);
      }
    }
  }
...
fn slot_in(
  ...
  let regions = region_shapes
    ...
    .map(|region| {
      ...
      let region_start =
        first_fit_region(&mut column_allocations, &region_columns, region.row_count(), 0, None)
...
fn test_slot_in() {
  ...
  assert_eq!(slot_in(regions).0.into_iter().map(|(i, _)| i).collect::<Vec<_>>(), vec![
    0.into(),
    0.into(),
    15.into()
  ]);
