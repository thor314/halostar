  ...
  Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
{
  ...
  let instance: Vec<InstanceSingle<Scheme::Curve>> = instances
    ...
    .map(|instance| -> Result<InstanceSingle<Scheme::Curve>, Error> {
      ...
      Ok(InstanceSingle { instance_values, instance_polys })
    })
    .collect::<Result<Vec<_>, _>>()?;
  ...
  let (advice, challenges) = {
    ...
    for current_phase in pk.vk.cs.phases() {
      ...
      for ((circuit, advice), instances) in circuits.iter().zip(advice.iter_mut()).zip(instances) {
        ...
        let blinds: Vec<_> =
          advice_values.iter().map(|_| Blind(Scheme::Scalar::random(&mut rng))).collect();
      ...
      for (index, phase) in meta.challenge_phase.iter().enumerate() {
        if current_phase == *phase {
          let existing = challenges.insert(index, *transcript.squeeze_challenge_scalar::<()>());
    ...
    let challenges =
      (0..meta.num_challenges).map(|index| challenges.remove(&index).unwrap()).collect::<Vec<_>>();
  ...
  if P::QUERY_INSTANCE {
    ...
    for instance in instance.iter() {
      ...
      let instance_evals: Vec<_> = meta
        ...
        .iter()
        .map(|&(column, at)| {
          eval_polynomial(&instance.instance_polys[column.index()], domain.rotate_omega(*x, at))
        })
        .collect();
  ...
  let lookups: Vec<Vec<lookup::prover::Evaluated<Scheme::Curve>>> = lookups
    .into_iter()
    .map(|lookups| -> Result<Vec<_>, _> {
      lookups.into_iter().map(|p| p.evaluate(pk, x, transcript)).collect::<Result<Vec<_>, _>>()
    })
    .collect::<Result<Vec<_>, _>>()?;
  ...
  let shuffles: Vec<Vec<shuffle::prover::Evaluated<Scheme::Curve>>> = shuffles
    ...
    .map(|shuffles| -> Result<Vec<_>, _> {
      shuffles.into_iter().map(|p| p.evaluate(pk, x, transcript)).collect::<Result<Vec<_>, _>>()
    })
    .collect::<Result<Vec<_>, _>>()?;
  ...
  prover.create_proof(rng, transcript, instances).map_err(|_| Error::ConstraintSystemFailure)
}
