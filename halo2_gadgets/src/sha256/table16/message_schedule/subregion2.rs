...
impl Subregion2Word {
  ...
  fn spread_c(&self) -> Value<[bool; 6]> { self.c.value().map(|v| v.spread()) }

  fn spread_d(&self) -> Value<[bool; 14]> { self.spread_d.value().map(|v| v.0) }

  fn spread_e(&self) -> Value<[bool; 2]> { self.e.value().map(|v| v.spread()) }

  fn spread_f(&self) -> Value<[bool; 2]> { self.f.value().map(|v| v.spread()) }

  fn spread_g(&self) -> Value<[bool; 26]> { self.spread_g.value().map(|v| v.0) }
...
impl MessageScheduleConfig {
  ...
  pub fn assign_subregion2(
    ...
    let mut new_word = |idx: usize,
     ...
     -> Result<Vec<(AssignedBits<16>, AssignedBits<16>)>, Error> {
      ...
      let (word, carry) = sum_with_carry(vec![
        ...
        (w_halves[new_word_idx - 7].0.value_u16(), w_halves[new_word_idx - 7].1.value_u16()),
        (sigma_0_output.0.value_u16(), sigma_0_output.1.value_u16()),
        (w_halves[new_word_idx - 16].0.value_u16(), w_halves[new_word_idx - 16].1.value_u16()),
      ]);
    ...
    for i in 27..49 {
      tmp_lower_sigma_0_v2_results = new_word(i, &tmp_lower_sigma_0_v2_results[i + 2 - 15 - 14])?;
    }
