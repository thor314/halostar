...
use crate::{
  ...
  multicore,
  plonk::{
    lookup, lookup::prover::Committed, permutation, permutation::Argument, AdviceQuery, Any,
    FixedQuery, InstanceQuery, ProvingKey,
  },
  poly::{
    commitment::Params, Basis, Coeff, EvaluationDomain, ExtendedLagrangeCoeff, LagrangeCoeff,
...
impl Default for ValueSource {
  fn default() -> Self { ValueSource::Constant(0) }
}
...
impl<C: CurveAffine> Evaluator<C> {
  ...
  pub fn new(cs: &ConstraintSystem<C::ScalarExt>) -> Self {
    ...
    for gate in cs.gates.iter() {
      parts.extend(gate.polynomials().iter().map(|poly| ev.custom_gates.add_expression(poly)));
    }
    ...
    for lookup in cs.lookups.iter() {
      ...
      let right_gamma =
        graph.add_calculation(Calculation::Add(compressed_table_coset, ValueSource::Gamma()));
      let lc = graph.add_calculation(Calculation::Add(compressed_input_coset, ValueSource::Beta()));
    ...
    for shuffle in cs.shuffles.iter() {
      ...
      let _ =
        graph_input.add_calculation(Calculation::Add(compressed_input_coset, ValueSource::Gamma()));

      let mut graph_shuffle = GraphEvaluator::default();
      let compressed_shuffle_coset = evaluate_lc(&shuffle.shuffle_expressions, &mut graph_shuffle);
      let _ = graph_shuffle
        .add_calculation(Calculation::Add(compressed_shuffle_coset, ValueSource::Gamma()));
  ...
  pub(in crate::plonk) fn evaluate_h(
    ...
    let advice: Vec<Vec<Polynomial<C::Scalar, ExtendedLagrangeCoeff>>> = advice_polys
      .iter()
      .map(|advice_polys| {
        advice_polys.iter().map(|poly| domain.coeff_to_extended(poly.clone())).collect()
      })
      .collect();
    let instance: Vec<Vec<Polynomial<C::Scalar, ExtendedLagrangeCoeff>>> = instance_polys
      .iter()
      .map(|instance_polys| {
        instance_polys.iter().map(|poly| domain.coeff_to_extended(poly.clone())).collect()
      })
      .collect();
      ...
      .zip(permutations.iter())
    {
              ...
              if set_idx != 0 {
                *value = *value * y
                  + ((set.permutation_product_coset[idx]
                    - permutation.sets[set_idx - 1].permutation_product_coset[r_last])
                    * l0[idx]);
              }
            }
      ...
      for (n, lookup) in lookups.iter().enumerate() {
        ...
        let permuted_input_coset =
          pk.vk.domain.coeff_to_extended(lookup.permuted_input_poly.clone());
        let permuted_table_coset =
          pk.vk.domain.coeff_to_extended(lookup.permuted_table_poly.clone());
  ...
  fn add_expression(&mut self, expr: &Expression<C::ScalarExt>) -> ValueSource {
    match expr {
      ...
      Expression::Fixed(query) => {
        ...
        self.add_calculation(Calculation::Store(ValueSource::Fixed(query.column_index, rot_idx)))
      },
      Expression::Advice(query) => {
        let rot_idx = self.add_rotation(&query.rotation);
        self.add_calculation(Calculation::Store(ValueSource::Advice(query.column_index, rot_idx)))
      },
      Expression::Instance(query) => {
        let rot_idx = self.add_rotation(&query.rotation);
        self.add_calculation(Calculation::Store(ValueSource::Instance(query.column_index, rot_idx)))
      },
