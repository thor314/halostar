...
impl<Fixed: FixedPoints<pallas::Affine>> Config<Fixed> {
  pub(crate) fn configure(
    ...
    let config = Self { q_mul_fixed_full: meta.selector(), super_config };
  ...
  fn witness(
    ...
    let windows = self
      .decompose_scalar_fixed::<{ pallas::Scalar::NUM_BITS as usize }>(scalar, offset, region)?;

    Ok(EccScalarFixed { value: scalar, windows: Some(windows) })
  }
  ...
  fn decompose_scalar_fixed<const SCALAR_NUM_BITS: usize>(
    ...
    let scalar_windows = scalar_windows
      .map(|windows| windows.into_iter().map(|window| pallas::Base::from(window as u64)))
  ...
  where
    <Fixed as FixedPoints<pallas::Affine>>::FullScalar: super::super::FixedPoint<pallas::Affine>,
  {
...
pub mod tests {
  ...
  fn test_single_base(
    ...
    // on the last step.
    {
      ...
      let zero =
        ScalarFixed::new(chip.clone(), layouter.namespace(|| "0"), Value::known(scalar_fixed))?;
    ...
    // [-1]B is the largest scalar field element.
    {
      ...
      let neg_1 =
        ScalarFixed::new(chip.clone(), layouter.namespace(|| "-1"), Value::known(scalar_fixed))?;
