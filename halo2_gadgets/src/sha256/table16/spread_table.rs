...
impl<const DENSE: usize, const SPREAD: usize> SpreadWord<DENSE, SPREAD> {
  pub(super) fn new(dense: [bool; DENSE]) -> Self {
    assert!(DENSE <= 16);
    SpreadWord { tag: get_tag(lebs2ip(&dense) as u16), dense, spread: spread_bits(dense) }
  }

  pub(super) fn try_new<T: TryInto<[bool; DENSE]> + std::fmt::Debug>(dense: T) -> Self
  where <T as TryInto<[bool; DENSE]>>::Error: std::fmt::Debug {
    assert!(DENSE <= 16);
...
impl<const DENSE: usize, const SPREAD: usize> SpreadVar<DENSE, SPREAD> {
  pub(super) fn with_lookup(
    ...
    let dense = AssignedBits::<DENSE>::assign_bits(region, || "dense", cols.dense, row, dense_val)?;
  ...
  pub(super) fn without_lookup(
    ...
    let dense =
      AssignedBits::<DENSE>::assign_bits(region, || "dense", dense_col, dense_row, dense_val)?;

    let spread =
      AssignedBits::<SPREAD>::assign_bits(region, || "spread", spread_col, spread_row, spread_val)?;
...
impl<F: PrimeField> SpreadTableChip<F> {
  pub fn configure(
    ...
    meta.lookup("lookup", |meta| {
      ...
      vec![(tag_cur, table_tag), (dense_cur, table_dense), (spread_cur, table_spread)]
    });

    SpreadTableConfig {
      input: SpreadInputs { tag: input_tag, dense: input_dense, spread: input_spread },
      table: SpreadTable { tag: table_tag, dense: table_dense, spread: table_spread },
    }
  }
      ...
      |mut table| {
        ...
        for index in 0..(1 << 16) {
          ...
          table.assign_cell(
            ...
            || {
              row = rows.next();
              Value::known(row.map(|(tag, ..)| tag).unwrap())
            },
          )?;
          table.assign_cell(
            || "dense",
        ...
        layouter.assign_region(
          ...
          |mut gate| {
            ...
            let mut add_row = |tag, dense, spread| -> Result<(), Error> {
              ...
              gate.assign_advice(|| "dense", config.input.dense, row, || Value::known(dense))?;
              gate.assign_advice(|| "spread", config.input.spread, row, || Value::known(spread))?;
              row += 1;
              Ok(())
            };
            ...
            // - 10-bit
            add_row(F::ONE, F::from(0b1111111111), F::from(0b01010101010101010101))?;
            add_row(F::from(2), F::from(0b10000000000), F::from(0b0100000000000000000000))?;
            // - 11-bit
            add_row(F::from(2), F::from(0b11111111111), F::from(0b0101010101010101010101))?;
            add_row(F::from(3), F::from(0b100000000000), F::from(0b010000000000000000000000))?;
            // - 13-bit
            add_row(F::from(3), F::from(0b1111111111111), F::from(0b01010101010101010101010101))?;
            add_row(
              F::from(4),
