...
pub struct MerklePath<
  ...
  const MAX_WORDS: usize,
  const PAR: usize,
> where MerkleChip: MerkleInstructions<C, PATH_LENGTH, K, MAX_WORDS> + Clone {
...
where MerkleChip: MerkleInstructions<C, PATH_LENGTH, K, MAX_WORDS> + Clone
{
  ...
  pub fn construct(
    ...
    assert_ne!(PAR, 0);
    Self { chips, domain, leaf_pos, path }
  }
}
...
pub mod tests {
  ...
  impl Circuit<pallas::Base> for MyCircuit {
    ...
    fn configure(meta: &mut ConstraintSystem<pallas::Base>) -> Self::Config {
      ...
      let lookup =
        (meta.lookup_table_column(), meta.lookup_table_column(), meta.lookup_table_column());
    ...
    fn synthesize(
      ...
      let leaf =
        chip_1.load_private(layouter.namespace(|| ""), config.0.cond_swap_config.a(), self.leaf)?;
      ...
      self
        ...
        .assert_if_known(|(((leaf, leaf_pos), merkle_path), computed_final_root)| {
          ...
          let final_root = merkle_path.iter().enumerate().fold(*leaf, |node, (l, sibling)| {
            let l = l as u8;
            let (left, right) =
              if leaf_pos & (1 << l) == 0 { (&node, sibling) } else { (sibling, &node) };
            ...
            merkle_crh
              .hash(
                iter::empty()
                  ...
                  .chain(left.to_le_bits().iter().by_vals().take(pallas::Base::NUM_BITS as usize))
                  .chain(right.to_le_bits().iter().by_vals().take(pallas::Base::NUM_BITS as usize)),
              )
  ...
  fn merkle_chip() {
    ...
    let path: Vec<_> = (0..(MERKLE_DEPTH)).map(|_| pallas::Base::random(rng)).collect();
