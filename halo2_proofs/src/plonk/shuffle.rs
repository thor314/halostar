...
impl<F: Field> Argument<F> {
  ...
  pub fn new<S: AsRef<str>>(name: S, shuffle: Vec<(Expression<F>, Expression<F>)>) -> Self {
    ...
    Argument { name: name.as_ref().to_string(), input_expressions, shuffle_expressions }
  }
  ...
  pub fn input_expressions(&self) -> &Vec<Expression<F>> { &self.input_expressions }

  /// Returns table of this argument
  pub fn shuffle_expressions(&self) -> &Vec<Expression<F>> { &self.shuffle_expressions }
