...
impl<F: WithSmallOrderMulGroup<3>> Argument<F> {
    ...
    C::Curve: Mul<F, Output = C::Curve> + MulAssign<F>,
  {
    ...
    Compressed { input_expression, shuffle_expression }
  }
    ...
    C::Curve: Mul<F, Output = C::Curve> + MulAssign<F>,
  {
    ...
    parallelize(&mut shuffle_product, |shuffle_product, start| {
      for (shuffle_product, shuffle_value) in
        shuffle_product.iter_mut().zip(compressed.shuffle_expression[start..].iter())
      {
    ...
    Ok(Committed::<C> { product_poly: z, product_blind })
  }
}
...
impl<C: CurveAffine> Committed<C> {
  pub(in crate::plonk) fn evaluate<E: EncodedChallenge<C>, T: TranscriptWrite<C, E>>(
    ...
    for eval in iter::empty().chain(Some(product_eval)).chain(Some(product_next_eval)) {
