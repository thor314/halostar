...
impl Assembly {
  ...
  pub fn copy(
    ...
    // Check bounds
    if left_row >= self.mapping[left_column].len() || right_row >= self.mapping[right_column].len()
    {
  ...
  pub fn columns(&self) -> &[Column<Any>] { &self.columns }
...
pub(crate) fn build_pk<'params, C: CurveAffine, P: Params<'params, C>>(
  ...
  ProvingKey { permutations, polys, cosets }
}
