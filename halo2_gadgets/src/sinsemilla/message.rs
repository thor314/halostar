  ...
  for Message<F, K, MAX_WORDS>
{
  ...
  fn deref(&self) -> &[MessagePiece<F, K>] { &self.0 }
}
...
impl<F: PrimeFieldBits, const K: usize> MessagePiece<F, K> {
  pub fn new(cell_value: AssignedCell<F, F>, num_words: usize) -> Self {
    ...
    Self { cell_value, num_words }
  }

  pub fn num_words(&self) -> usize { self.num_words }

  pub fn cell(&self) -> Cell { self.cell_value.cell() }

  pub fn field_elem(&self) -> Value<F> { self.cell_value.value().cloned() }

  pub fn cell_value(&self) -> AssignedCell<F, F> { self.cell_value.clone() }
}
