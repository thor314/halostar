...
impl From<pallas::Point> for IncompletePoint {
  fn from(p: pallas::Point) -> Self { IncompletePoint(CtOption::new(p, 1.into())) }
}

impl From<IncompletePoint> for CtOption<pallas::Point> {
  fn from(p: IncompletePoint) -> Self { p.0 }
}
  ...
  fn add(self, rhs: Self) -> Self::Output {
    ...
    IncompletePoint(self.0.and_then(|p| {
      ...
      rhs.0.and_then(|q| {
        ...
        CtOption::new(p + q, !(p.is_identity() | q.is_identity() | p.ct_eq(&q) | p.ct_eq(&-q)))
      })
    }))
  }
}
