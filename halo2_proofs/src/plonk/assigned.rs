...
impl<F: Field> From<&Assigned<F>> for Assigned<F> {
  fn from(val: &Assigned<F>) -> Self { *val }
}

impl<F: Field> From<&F> for Assigned<F> {
  fn from(numerator: &F) -> Self { Assigned::Trivial(*numerator) }
}
...
impl<F: Field> From<(F, F)> for Assigned<F> {
  fn from((numerator, denominator): (F, F)) -> Self { Assigned::Rational(numerator, denominator) }
}
...
impl<F: Field> Add<F> for Assigned<F> {
  ...
  fn add(self, rhs: F) -> Assigned<F> { self + Self::Trivial(rhs) }
}

impl<F: Field> Add<F> for &Assigned<F> {
  type Output = Assigned<F>;

  fn add(self, rhs: F) -> Assigned<F> { *self + rhs }
}

impl<F: Field> Add<&Assigned<F>> for Assigned<F> {
  type Output = Assigned<F>;

  fn add(self, rhs: &Self) -> Assigned<F> { self + *rhs }
}

impl<F: Field> Add<Assigned<F>> for &Assigned<F> {
  type Output = Assigned<F>;

  fn add(self, rhs: Assigned<F>) -> Assigned<F> { *self + rhs }
}
...
impl<F: Field> Sub for Assigned<F> {
  ...
  fn sub(self, rhs: Assigned<F>) -> Assigned<F> { self + (-rhs) }
}

impl<F: Field> Sub<F> for Assigned<F> {
  type Output = Assigned<F>;

  fn sub(self, rhs: F) -> Assigned<F> { self + (-rhs) }
}
...
impl<F: Field> Sub<&Assigned<F>> for Assigned<F> {
  ...
  fn sub(self, rhs: &Self) -> Assigned<F> { self - *rhs }
}
...
impl<F: Field> Mul<F> for Assigned<F> {
  ...
  fn mul(self, rhs: F) -> Assigned<F> { self * Self::Trivial(rhs) }
}

impl<F: Field> Mul<F> for &Assigned<F> {
  type Output = Assigned<F>;

  fn mul(self, rhs: F) -> Assigned<F> { *self * rhs }
}

impl<F: Field> Mul<&Assigned<F>> for Assigned<F> {
  type Output = Assigned<F>;

  fn mul(self, rhs: &Assigned<F>) -> Assigned<F> { self * *rhs }
}
...
impl<F: Field> Assigned<F> {
  ...
  pub fn double(&self) -> Self {
    match self {
      ...
      Self::Rational(numerator, denominator) => Self::Rational(numerator.double(), *denominator),
    }
  }
  ...
  /// Cubes this element.
  #[must_use]
  pub fn cube(&self) -> Self { self.square() * self }
...
mod proptests {
  ...
  impl<F: Field> UnaryOperand for F {
    ...
    fn cube(&self) -> Self { self.cube() }

    fn inv0(&self) -> Self { self.invert().unwrap_or(F::ZERO) }
  }
