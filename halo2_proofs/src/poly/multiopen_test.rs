...
mod test {
  ...
  use crate::{
    ...
    poly::{
      commitment::{
        Blind, CommitmentScheme, Params, ParamsProver, ParamsVerifier, Prover, Verifier, MSM,
      },
      query::{PolynomialPointer, ProverQuery, VerifierQuery},
  ...
  fn test_roundtrip_gwc() {
    ...
    let proof = create_proof::<_, ProverGWC<_>, _, Blake2bWrite<_, _, Challenge255<_>>>(&params);
  ...
  where Scheme::Scalar: WithSmallOrderMulGroup<3> {
