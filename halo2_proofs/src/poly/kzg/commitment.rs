...
use crate::{
  arithmetic::{best_fft, best_multiexp, g_to_lagrange, parallelize, CurveAffine, CurveExt},
  helpers::SerdeCurveAffine,
  poly::{
    commitment::{Blind, CommitmentScheme, Params, ParamsProver, ParamsVerifier, MSM},
  ...
  E::G2Affine: SerdeCurveAffine,
{
  ...
  fn new_params(k: u32) -> Self::ParamsProver { ParamsKZG::new(k) }
          ...
          |reader: &mut R| -> io::Result<Vec<Option<E::G1Affine>>> {
            ...
            parallelize(&mut points, |points, chunks| {
              for (i, point) in points.iter_mut().enumerate() {
                *point = Option::from(E::G1Affine::from_bytes(&points_compressed[chunks + i]));
              }
            });
            Ok(points)
          };
...
// So we probably need much smaller verifier key. However for new bases in g1 should be in verifier
// keys.
  ...
  E::G2Affine: SerdeCurveAffine,
{
  ...
  fn empty_msm(&'params self) -> MSMKZG<E> { MSMKZG::new() }
  ...
  E::G2Affine: SerdeCurveAffine,
{
  ...
  fn verifier_params(&'params self) -> &'params Self::ParamsVerifier { self }

  fn new(k: u32) -> Self { Self::setup(k, OsRng) }
...
mod test {
  ...
  use crate::{
    arithmetic::{best_fft, best_multiexp, parallelize, CurveAffine, CurveExt},
    poly::{
      commitment::{Blind, CommitmentScheme, Params, ParamsProver, MSM},
      kzg::{
