...
#[cfg(test)] pub use schedule_util::msg_schedule_test_input;
use schedule_util::*;
...
impl MessageScheduleConfig {
  ...
  ) -> Result<([MessageWord; ROUNDS], [(AssignedBits<16>, AssignedBits<16>); ROUNDS]), Error> {
    ...
    layouter.assign_region(
      ...
      |mut region| {
        ...
        for index in 14..49 {
          ...
          self.s_lower_sigma_1_v2.enable(&mut region, row + SIGMA_0_V2_ROWS + 3)?;

          let new_word_idx = index + 2;
          self.s_word.enable(&mut region, get_word_row(new_word_idx - 16) + 1)?;
        }
