...
where S: Spec<Fp, WIDTH, RATE> + Clone + Copy {
  message: Value<[Fp; L]>,
  _spec:   PhantomData<S>,
}
...
where S: Spec<Fp, WIDTH, RATE> + Copy + Clone
{
  ...
  fn without_witnesses(&self) -> Self { Self { message: Value::unknown(), _spec: PhantomData } }
    ...
    let message = layouter.assign_region(
      ...
      |mut region| {
        let message_word = |i: usize| {
          ...
          region.assign_advice(|| format!("load message_{}", i), config.input[i], 0, || value)
        };
  ...
  S: Spec<Fp, WIDTH, RATE> + Copy + Clone,
{
  ...
  let empty_circuit =
    HashCircuit::<S, WIDTH, RATE, L> { message: Value::unknown(), _spec: PhantomData };
  ...
  let mut rng = OsRng;
  let message: [Fp; L] =
    (0..L).map(|_| pallas::Base::random(rng)).collect::<Vec<_>>().try_into().unwrap();
  let output = poseidon::Hash::<_, S, ConstantLength<L>, WIDTH, RATE>::init().hash(message);

  let circuit =
    HashCircuit::<S, WIDTH, RATE, L> { message: Value::known(message), _spec: PhantomData };
