  ...
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Error::NotEnoughColumnsForConstants => {
        write!(f, "Too few fixed columns are enabled for global constants usage")
      },
      Error::ColumnNotInPermutation(column) => write!(
        f,
        "Column {:?} must be included in the permutation. Help: try applying \
         `meta.enable_equalty` on the column",
        column
      ),
    }
  }
