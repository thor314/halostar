...
use halo2_proofs::{
  ...
  plonk::{
    Advice, Column, ConstraintSystem, Constraints, Error, Expression, Fixed, Selector, TableColumn,
    VirtualCells,
  },
  poly::Rotation,
};
  ...
  Commit: CommitDomains<pallas::Affine, F, Hash>,
{
  ...
  pub fn configure(
    ...
    let x_r =
      |meta: &mut VirtualCells<pallas::Base>, rotation| config.double_and_add.x_r(meta, rotation);

    // Y_A = (lambda_1 + lambda_2) * (x_a - x_r)
    let Y_A =
      |meta: &mut VirtualCells<pallas::Base>, rotation| config.double_and_add.Y_A(meta, rotation);
    ...
    meta.create_gate("Sinsemilla gate", |meta| {
      ...
      let secant_line = lambda_2_cur.clone().square() - (x_a_next.clone() + x_r + x_a_cur.clone());
  ...
  Commit: CommitDomains<pallas::Affine, F, Hash>,
{
  ...
  fn witness_message_piece(
    ...
    let cell = layouter.assign_region(
      ...
      |mut region| {
        region.assign_advice(|| "witness message piece", config.witness_pieces, 0, || field_elem)
      },
    )?;
  ...
  fn hash_to_point(
    ...
    layouter
      .assign_region(|| "hash_to_point", |mut region| self.hash_message(&mut region, Q, &message))
  }

  fn extract(point: &Self::NonIdentityPoint) -> Self::X { point.x() }
}
