use std::{cmp, collections::HashSet, ops::Range};
...
impl CircuitLayout {
  ...
  pub fn render<F: Field, ConcreteCircuit: Circuit<F>, DB: DrawingBackend>(
    ...
    root.draw(&Rectangle::new(
      [(cs.num_instance_columns + cs.num_advice_columns, 0), (total_columns, view_bottom)],
      ShapeStyle::from(&BLUE.mix(0.2)).filled(),
    ))?;
    {
      root.draw(&Rectangle::new(
        [
          (cs.num_instance_columns + cs.num_advice_columns + non_selector_fixed_columns, 0),
    ...
    root.draw(&Rectangle::new([(0, 0), (total_columns, view_bottom)], BLACK))?;

    let draw_region = |root: &DrawingArea<_, _>, top_left, bottom_right| {
      root.draw(&Rectangle::new([top_left, bottom_right], ShapeStyle::from(&WHITE).filled()))?;
      root.draw(&Rectangle::new(
    ...
    if self.show_equality_constraints {
      for (l_col, l_row, r_col, r_row) in &layout.equality {
        let l_col = column_index(&cs, (*l_col).into());
        let r_col = column_index(&cs, (*r_col).into());
        root.draw(&PathElement::new([(l_col, *l_row), (r_col, *r_row)], ShapeStyle::from(&RED)))?;
      }
    }
