...
use super::{
  super::{CommitDomains, HashDomains, SinsemillaInstructions},
  NonIdentityEccPoint, SinsemillaChip,
};
use crate::{
  ecc::FixedPoints,
  ...
  Commit: CommitDomains<pallas::Affine, Fixed, Hash>,
{
  ...
  pub(super) fn hash_message(
    ...
    message: &<Self as SinsemillaInstructions<
      ...
      { sinsemilla::C },
    >>::Message,
  ) -> Result<(NonIdentityEccPoint, Vec<Vec<AssignedCell<pallas::Base, pallas::Base>>>), Error> {
    ...
    let mut y_a: Y<pallas::Base> = {
      ...
      region.assign_fixed(|| "fixed y_q", config.fixed_y_q, offset, || Value::known(y_q))?;
    ...
    // Check equivalence to result from primitives::sinsemilla::hash_to_point
    {
      ...
      field_elems.zip(x_a.value().zip(y_a.value())).assert_if_known(|(field_elems, (x_a, y_a))| {
        ...
        let expected_point =
          bitstring.chunks(K).fold(Q.to_curve(), |acc, chunk| (acc + S(chunk)) + acc);
  ...
  fn hash_piece(
    ...
    (X<pallas::Base>, Y<pallas::Base>, Vec<AssignedCell<pallas::Base, pallas::Base>>),
    Error,
  > {
    let config = self.config().clone();
    ...
    // Selector assignments
    {
      ...
      region.assign_fixed(
        ...
        || Value::known(if final_piece { pallas::Base::from(2) } else { pallas::Base::zero() }),
      )?;
    }

    // Message piece as K * piece.length bitstring
    let bitstring: Value<Vec<bool>> = piece.field_elem().map(|value| {
      value.to_le_bits().into_iter().take(sinsemilla::K * piece.num_words()).collect()
    });

    let words: Value<Vec<u32>> =
      bitstring.map(|bitstring| bitstring.chunks_exact(sinsemilla::K).map(lebs2ip_k).collect());

    // Get (x_p, y_p) for each word.
    let generators: Value<Vec<(pallas::Base, pallas::Base)>> =
      words.clone().map(|words| words.iter().map(|word| SINSEMILLA_S[*word as usize]).collect());
...
impl<F: Field> From<AssignedCell<Assigned<F>, F>> for X<F> {
  fn from(cell_value: AssignedCell<Assigned<F>, F>) -> Self { X(cell_value) }
}

impl<F: Field> Deref for X<F> {
  type Target = AssignedCell<Assigned<F>, F>;

  fn deref(&self) -> &AssignedCell<Assigned<F>, F> { &self.0 }
}
...
impl<F: Field> From<Value<Assigned<F>>> for Y<F> {
  fn from(value: Value<Assigned<F>>) -> Self { Y(value) }
}

impl<F: Field> Deref for Y<F> {
  type Target = Value<Assigned<F>>;

  fn deref(&self) -> &Value<Assigned<F>> { &self.0 }
}
