...
impl From<InitialRound> for RoundIdx {
  fn from(_: InitialRound) -> Self { RoundIdx::Init }
}

impl From<MainRoundIdx> for RoundIdx {
  fn from(idx: MainRoundIdx) -> Self { RoundIdx::Main(idx) }
}
...
impl From<usize> for MainRoundIdx {
  fn from(idx: usize) -> Self { MainRoundIdx(idx) }
}

impl std::ops::Add<usize> for MainRoundIdx {
  type Output = Self;

  fn add(self, rhs: usize) -> Self::Output { MainRoundIdx(self.0 + rhs) }
}
...
pub fn get_decompose_e_row(round_idx: RoundIdx) -> usize { get_round_row(round_idx) }
...
pub fn get_ch_neg_row(round_idx: MainRoundIdx) -> usize { get_ch_row(round_idx) + CH_ROWS / 2 }
...
pub fn get_h_prime_row(round_idx: MainRoundIdx) -> usize { get_ch_row(round_idx) }
...
pub fn get_e_new_row(round_idx: MainRoundIdx) -> usize { get_d_row(round_idx.into()) }

pub fn get_a_new_row(round_idx: MainRoundIdx) -> usize { get_maj_row(round_idx) }

pub fn get_digest_abcd_row() -> usize { SUBREGION_MAIN_ROWS }

pub fn get_digest_efgh_row() -> usize { get_digest_abcd_row() + 2 }
...
impl CompressionConfig {
  pub(super) fn decompose_abcd(
    ...
    Ok(AbcdVar { a, b, c_lo, c_mid, c_hi, d })
  }
  ...
  pub(super) fn decompose_efgh(
    ...
    Ok(EfghVar { a_lo, a_hi, b_lo, b_hi, c, d })
  }
  ...
  pub(super) fn assign_upper_sigma_0(
    ...
    word.a.spread.copy_advice(|| "spread_a", region, a_3, row + 1)?;
    ...
    word.c_lo.spread.copy_advice(|| "spread_c_lo", region, a_3, row - 1)?;
    // Assign `spread_c_mid` and copy constraint
    word.c_mid.spread.copy_advice(|| "spread_c_mid", region, a_4, row - 1)?;
    // Assign `spread_c_hi` and copy constraint
    word.c_hi.spread.copy_advice(|| "spread_c_hi", region, a_4, row + 1)?;
    ...
    self.assign_sigma_outputs(region, &self.lookup, a_3, row, r_0_even, r_0_odd, r_1_even, r_1_odd)
  }
  ...
  pub(super) fn assign_upper_sigma_1(
    ...
    word.a_lo.spread.copy_advice(|| "spread_a_lo", region, a_3, row + 1)?;
    // Assign `spread_a_hi` and copy constraint
    word.a_hi.spread.copy_advice(|| "spread_a_hi", region, a_4, row + 1)?;
    // Assign `spread_b_lo` and copy constraint
    word.b_lo.spread.copy_advice(|| "spread_b_lo", region, a_3, row - 1)?;
    // Assign `spread_b_hi` and copy constraint
    word.b_hi.spread.copy_advice(|| "spread_b_hi", region, a_4, row - 1)?;
  ...
  pub(super) fn assign_ch(
    ...
    spread_halves_e.0.copy_advice(|| "spread_e_lo", region, a_3, row - 1)?;
    spread_halves_e.1.copy_advice(|| "spread_e_hi", region, a_4, row - 1)?;

    // Assign and copy spread_f_lo, spread_f_hi
    spread_halves_f.0.copy_advice(|| "spread_f_lo", region, a_3, row + 1)?;
    spread_halves_f.1.copy_advice(|| "spread_f_hi", region, a_4, row + 1)?;

    let p: Value<[bool; 64]> =
      spread_halves_e.value().zip(spread_halves_f.value()).map(|(e, f)| i2lebsp(e + f));
  ...
  pub(super) fn assign_ch_neg(
    ...
    // Assign and copy spread_e_lo, spread_e_hi
    spread_halves_e.0.copy_advice(|| "spread_e_lo", region, a_5, row - 1)?;
    spread_halves_e.1.copy_advice(|| "spread_e_hi", region, a_5, row)?;

    // Assign and copy spread_g_lo, spread_g_hi
    spread_halves_g.0.copy_advice(|| "spread_g_lo", region, a_3, row + 1)?;
    spread_halves_g.1.copy_advice(|| "spread_g_hi", region, a_4, row + 1)?;

    // Calculate neg_e_lo
    let spread_neg_e_lo = spread_halves_e.0.value().map(|spread_e_lo| negate_spread(spread_e_lo.0));
    // Assign spread_neg_e_lo
    AssignedBits::<32>::assign_bits(region, || "spread_neg_e_lo", a_3, row - 1, spread_neg_e_lo)?;

    // Calculate neg_e_hi
    let spread_neg_e_hi = spread_halves_e.1.value().map(|spread_e_hi| negate_spread(spread_e_hi.0));
    // Assign spread_neg_e_hi
    AssignedBits::<32>::assign_bits(region, || "spread_neg_e_hi", a_4, row - 1, spread_neg_e_hi)?;

    let p: Value<[bool; 64]> = {
      let spread_neg_e = spread_neg_e_lo
        .zip(spread_neg_e_hi)
        .map(|(lo, hi)| lebs2ip(&lo) + (1 << 32) * lebs2ip(&hi));
      spread_neg_e.zip(spread_halves_g.value()).map(|(neg_e, g)| i2lebsp(neg_e + g))
    };
  ...
  pub(super) fn assign_maj(
    ...
    spread_halves_a.0.copy_advice(|| "spread_a_lo", region, a_4, row - 1)?;
    spread_halves_a.1.copy_advice(|| "spread_a_hi", region, a_5, row - 1)?;

    // Assign and copy spread_b_lo, spread_b_hi
    spread_halves_b.0.copy_advice(|| "spread_b_lo", region, a_4, row)?;
    spread_halves_b.1.copy_advice(|| "spread_b_hi", region, a_5, row)?;

    // Assign and copy spread_c_lo, spread_c_hi
    spread_halves_c.0.copy_advice(|| "spread_c_lo", region, a_4, row + 1)?;
    spread_halves_c.1.copy_advice(|| "spread_c_hi", region, a_5, row + 1)?;
  ...
  pub(super) fn assign_h_prime(
    ...
    // Assign h_prime_lo, h_prime_hi, h_prime_carry
    {
      let (h_prime, h_prime_carry) = sum_with_carry(vec![
        ...
        (Value::known(lebs2ip(&k_lo) as u16), Value::known(lebs2ip(&k_hi) as u16)),
  ...
  pub(super) fn assign_e_new(
    ...
    region.assign_advice(|| "e_new_carry", a_9, row + 1, || e_new_carry.map(pallas::Base::from))?;
  ...
  pub(super) fn assign_a_new(
    ...
    sigma_0.1.copy_advice(|| "sigma_0_hi", region, a_6, row + 1)?;

    // Assign and copy h_prime
    h_prime.0.copy_advice(|| "h_prime_lo", region, a_7, row - 1)?;
    h_prime.1.copy_advice(|| "h_prime_hi", region, a_8, row - 1)?;
    ...
    region.assign_advice(|| "a_new_carry", a_9, row, || a_new_carry.map(pallas::Base::from))?;
  ...
  pub fn assign_word_halves(
    ...
    Ok(((w_lo.dense, w_hi.dense).into(), (w_lo.spread, w_hi.spread).into()))
  }
}
