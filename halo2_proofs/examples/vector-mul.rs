...
impl<F: Field> FieldChip<F> {
  fn construct(config: <Self as Chip<F>>::Config) -> Self { Self { config, _marker: PhantomData } }
    ...
    layouter.assign_region(
      ...
      |mut region| {
        values
          ...
          .enumerate()
          .map(|(i, value)| {
            region.assign_advice(|| "private input", config.advice[0], i, || *value).map(Number)
          })
          .collect()
      },
    )
        ...
        |region: Region<'_, F>| {
          ...
          a.par_iter()
            ...
            .map(|(i, (a, b))| {
              ...
              region.assign_advice(|| "lhs * rhs", config.advice[2], i, || value).map(Number)
            })
            .collect()
        },
      )
