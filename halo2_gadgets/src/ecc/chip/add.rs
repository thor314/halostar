...
impl Config {
  ...
  pub(super) fn configure(
    ...
    let config =
      Self { q_add: meta.selector(), x_p, y_p, x_qr, y_qr, lambda, alpha, beta, gamma, delta };
  ...
  fn create_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    ...
    meta.create_gate("complete addition", |meta| {
      ...
      // (λ^2 - x_p - x_q - x_r)
      let nonexceptional_x_r = lambda.clone().square() - x_p.clone() - x_q.clone() - x_r.clone();
      // (λ ⋅(x_p - x_r) - y_p - y_r)
      let nonexceptional_y_r = lambda * x_p_minus_x_r - y_p.clone() - y_r.clone();

      // x_p⋅x_q⋅(x_q - x_p)⋅(λ^2 - x_p - x_q - x_r) = 0
      let poly3a = x_p.clone() * x_q.clone() * x_q_minus_x_p.clone() * nonexceptional_x_r.clone();
  ...
  pub(super) fn assign_region(
    ...
    let delta = x_p.zip(x_q).zip(y_p).zip(y_q).map(|(((x_p, x_q), y_p), y_q)| {
      if x_q == x_p {
        (y_q + y_p).invert()
      } else {
        Assigned::Zero
    ...
    // Assign lambda
    let lambda =
      x_p.zip(y_p).zip(x_q).zip(y_q).zip(alpha).map(|((((x_p, y_p), x_q), y_q), alpha)| {
        if x_q != x_p {
          // λ = (y_q - y_p)/(x_q - x_p)
    ...
    // Calculate (x_r, y_r)
    let r = x_p.zip(y_p).zip(x_q).zip(y_q).zip(lambda).map(|((((x_p, y_p), x_q), y_q), lambda)| {
      {
    ...
    let result = EccPoint { x: x_r_cell, y: y_r_cell };
    ...
    // Check that the correct sum is obtained.
    {
      ...
      real_sum.zip(result).assert_if_known(|(real_sum, result)| &real_sum.to_affine() == result);
    }
