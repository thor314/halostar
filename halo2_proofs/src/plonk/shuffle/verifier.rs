...
use super::{
  super::{circuit::Expression, ChallengeGamma, ChallengeTheta, ChallengeX},
  Argument,
};
use crate::{
...
impl<C: CurveAffine> Committed<C> {
  pub(crate) fn evaluate<E: EncodedChallenge<C>, T: TranscriptRead<C, E>>(
    ...
    Ok(Evaluated { committed: self, product_eval, product_next_eval })
  }
}
...
impl<C: CurveAffine> Evaluated<C> {
  pub(in crate::plonk) fn expressions<'a>(
    ...
    let product_expression = || {
      ...
      // z(\omega X) (s(X) + \gamma)
      let left =
        self.product_next_eval * &(compress_expressions(&argument.shuffle_expressions) + &*gamma);
      // z(X) (a(X) + \gamma)
      let right =
