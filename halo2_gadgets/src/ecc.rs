...
pub trait EccInstructions<C: CurveAffine>:
  ...
  /// Performs fixed-base scalar multiplication using a full-width scalar, returning `[scalar]
  /// base`.
  fn mul_fixed(
    &self,
    layouter: &mut impl Layouter<C::Base>,
...
impl<C: CurveAffine, EccChip: EccInstructions<C>> NonIdentityPoint<C, EccChip> {
  ...
  pub fn inner(&self) -> &EccChip::NonIdentityPoint { &self.inner }
  ...
  pub fn add<Other: Into<Point<C, EccChip>> + Clone>(
    ...
    self
      ...
      .map(|inner| Point { chip: self.chip.clone(), inner })
  }
  ...
  pub fn add_incomplete(
    ...
    self
      ...
      .map(|inner| NonIdentityPoint { chip: self.chip.clone(), inner })
  }
  ...
  pub fn mul(
    ...
    self.chip.mul(&mut layouter, &by.inner, &self.inner.clone()).map(|(point, scalar)| {
      (Point { chip: self.chip.clone(), inner: point }, ScalarVar {
  ...
  From<NonIdentityPoint<C, EccChip>> for Point<C, EccChip>
{
  fn from(non_id_point: NonIdentityPoint<C, EccChip>) -> Self {
    Self { chip: non_id_point.chip, inner: non_id_point.inner.into() }
  }
}
...
impl<C: CurveAffine, EccChip: EccInstructions<C> + Clone + Debug + Eq> Point<C, EccChip> {
  ...
  pub fn constrain_equal<Other: Into<Point<C, EccChip>> + Clone>(
    ...
    let other: Point<C, EccChip> = (other.clone()).into();
    self.chip.constrain_equal(&mut layouter, &self.inner, &other.inner)
  }

  /// Returns the inner point.
  pub fn inner(&self) -> &EccChip::Point { &self.inner }
  ...
  /// Wraps the given point (obtained directly from an instruction) in a gadget.
  pub fn from_inner(chip: EccChip, inner: EccChip::Point) -> Self { Point { chip, inner } }
...
impl<C: CurveAffine, EccChip: EccInstructions<C>> X<C, EccChip> {
  ...
  pub fn from_inner(chip: EccChip, inner: EccChip::X) -> Self { X { chip, inner } }

  /// Returns the inner x-coordinate.
  pub fn inner(&self) -> &EccChip::X { &self.inner }
}
...
impl<C: CurveAffine, EccChip: EccInstructions<C>> FixedPoint<C, EccChip> {
  ...
  pub fn mul(
    ...
    assert_eq!(self.chip, by.chip);
    self.chip.mul_fixed(&mut layouter, &by.inner, &self.inner).map(|(point, scalar)| {
      (Point { chip: self.chip.clone(), inner: point }, ScalarFixed {
        chip:  self.chip.clone(),
        inner: scalar,
      })
    })
...
impl<C: CurveAffine, EccChip: EccInstructions<C>> FixedPointShort<C, EccChip> {
  ...
  pub fn mul(
    ...
    assert_eq!(self.chip, by.chip);
    self.chip.mul_fixed_short(&mut layouter, &by.inner, &self.inner).map(|(point, scalar)| {
      (Point { chip: self.chip.clone(), inner: point }, ScalarFixedShort {
        chip:  self.chip.clone(),
        inner: scalar,
      })
    })
...
pub(crate) mod tests {
  ...
  impl FullWidth {
    pub(crate) fn from_pallas_generator() -> Self { FullWidth(*BASE, &ZS_AND_US) }
  ...
  impl FixedPoint<pallas::Affine> for FullWidth {
    ...
    fn generator(&self) -> pallas::Affine { self.0 }
    ...
    fn z(&self) -> Vec<u64> { self.1.iter().map(|(z, _)| *z).collect() }
  }

  impl FixedPoint<pallas::Affine> for BaseField {
    type FixedScalarKind = BaseFieldElem;

    fn generator(&self) -> pallas::Affine { *BASE }
    ...
    fn z(&self) -> Vec<u64> { ZS_AND_US.iter().map(|(z, _)| *z).collect() }
  }
  ...
  impl FixedPoint<pallas::Affine> for Short {
    ...
    fn z(&self) -> Vec<u64> { ZS_AND_US_SHORT.iter().map(|(z, _)| *z).collect() }
  }
  ...
  impl Circuit<pallas::Base> for MyCircuit {
    ...
    fn without_witnesses(&self) -> Self { MyCircuit { test_errors: false } }
  ...
  fn print_ecc_chip() {
    ...
    halo2_proofs::dev::CircuitLayout::default().render(13, &circuit, &root).unwrap();
  }
}
