...
>() -> (Vec<[F; T]>, Mds<F, T>, Mds<F, T>) {
  ...
  let round_constants = (0..(r_f + r_p))
    .map(|_| {
      ...
      for (rc, value) in rc_row.iter_mut().zip((0..T).map(|_| grain.next_field_element())) {
        *rc = value;
      }
      rc_row
    })
...
impl<F: fmt::Debug, const RATE: usize> Absorbing<F, RATE> {
  pub(crate) fn init_with(val: F) -> Self {
    Self(
      iter::once(Some(val)).chain((1..RATE).map(|_| None)).collect::<Vec<_>>().try_into().unwrap(),
    )
  }
}
  ...
  Sponge<F, S, Absorbing<F, RATE>, T, RATE>
{
  ...
  pub(crate) fn new(initial_capacity_element: F) -> Self {
    ...
    Sponge { mode, state, mds_matrix, round_constants, _marker: PhantomData }
  }
...
impl<F: PrimeField, const RATE: usize, const L: usize> Domain<F, RATE> for ConstantLength<L> {
  ...
  fn name() -> String { format!("ConstantLength<{}>", L) }
  ...
  Hash<F, S, D, T, RATE>
{
  ...
  pub fn init() -> Self {
    Hash { sponge: Sponge::new(D::initial_capacity_element()), _domain: PhantomData }
  }
}
  ...
  Hash<F, S, ConstantLength<L>, T, RATE>
{
  ...
  pub fn hash(mut self, message: [F; L]) -> F {
    for value in message.into_iter().chain(<ConstantLength<L> as Domain<F, RATE>>::padding(L)) {
