...
impl<C: ColumnType> Column<C> {
  #[cfg(test)]
  pub(crate) fn new(index: usize, column_type: C) -> Self { Column { index, column_type } }

  /// Index of this column.
  pub fn index(&self) -> usize { self.index }

  /// Type of this column.
  pub fn column_type(&self) -> &C { &self.column_type }
  ...
  pub fn cur<F: Field>(&self) -> Expression<F> { self.query_cell(Rotation::cur()) }

  /// Return expression from column at the next row
  pub fn next<F: Field>(&self) -> Expression<F> { self.query_cell(Rotation::next()) }

  /// Return expression from column at the previous row
  pub fn prev<F: Field>(&self) -> Expression<F> { self.query_cell(Rotation::prev()) }
...
pub(crate) mod sealed {
  ...
  impl Phase {
    pub fn prev(&self) -> Option<Phase> { self.0.checked_sub(1).map(Phase) }
  }

  impl SealedPhase for Phase {
    fn to_sealed(self) -> Phase { self }
  }
...
impl SealedPhase for super::FirstPhase {
  fn to_sealed(self) -> sealed::Phase { sealed::Phase(0) }
}
...
impl SealedPhase for super::SecondPhase {
  fn to_sealed(self) -> sealed::Phase { sealed::Phase(1) }
}
...
impl SealedPhase for super::ThirdPhase {
  fn to_sealed(self) -> sealed::Phase { sealed::Phase(2) }
}
...
impl Default for Advice {
  fn default() -> Advice { Advice { phase: FirstPhase.to_sealed() } }
}

impl Advice {
  /// Returns `Advice` in given `Phase`
  pub fn new<P: Phase>(phase: P) -> Advice { Advice { phase: phase.to_sealed() } }

  /// Phase of this column
  pub fn phase(&self) -> u8 { self.phase.0 }
}
...
impl Any {
  ...
  pub fn advice() -> Any { Any::Advice(Advice::default()) }

  /// Returns Advice variant in given `Phase`
  pub fn advice_in<P: Phase>(phase: P) -> Any { Any::Advice(Advice::new(phase)) }
}
...
impl ColumnType for Fixed {
  fn query_cell<F: Field>(&self, index: usize, at: Rotation) -> Expression<F> {
    Expression::Fixed(FixedQuery { index: None, column_index: index, rotation: at })
  }
}
...
impl TryFrom<Column<Any>> for Column<Advice> {
  ...
  fn try_from(any: Column<Any>) -> Result<Self, Self::Error> {
    match any.column_type() {
      Any::Advice(advice) => Ok(Column { index: any.index(), column_type: *advice }),
...
impl TryFrom<Column<Any>> for Column<Fixed> {
  ...
  fn try_from(any: Column<Any>) -> Result<Self, Self::Error> {
    match any.column_type() {
      Any::Fixed => Ok(Column { index: any.index(), column_type: Fixed }),
...
impl TryFrom<Column<Any>> for Column<Instance> {
  ...
  fn try_from(any: Column<Any>) -> Result<Self, Self::Error> {
    match any.column_type() {
      Any::Instance => Ok(Column { index: any.index(), column_type: Instance }),
...
impl Selector {
  ...
  pub fn is_simple(&self) -> bool { self.1 }

  /// Returns index of this selector
  pub fn index(&self) -> usize { self.0 }

  /// Return expression from selector
  pub fn expr<F: Field>(&self) -> Expression<F> { Expression::Selector(*self) }
}
...
impl FixedQuery {
  /// Column index
  pub fn column_index(&self) -> usize { self.column_index }
...
impl TableColumn {
  ...
  pub fn inner(&self) -> Column<Fixed> { self.inner }
}
...
impl Challenge {
  ...
  /// Return Expression
  pub fn expr<F: Field>(&self) -> Expression<F> { Expression::Challenge(*self) }
}
...
pub trait Assignment<F: Field> {
  ...
  ///
  /// Returns `Value::unknown()` if the current synthesis phase is before the challenge can be
  /// queried.
...
pub trait FloorPlanner {
  ...
  /// - Perform any necessary setup or measurement tasks, which may involve one or more calls to
...
pub trait Circuit<F: Field> {
  ...
  #[cfg(feature = "circuit-params")]
  fn params(&self) -> Self::Params { Self::Params::default() }
...
impl<F: Field> Expression<F> {
  ...
  pub fn query_cells(&mut self, cells: &mut VirtualCells<'_, F>) {
    match self {
      ...
      Expression::Advice(query) =>
        if query.index.is_none() {
          let col =
            Column { index: query.column_index, column_type: Advice { phase: query.phase } };
          cells.queried_cells.push((col, query.rotation).into());
  ...
  pub fn square(self) -> Self { self.clone() * self }
  ...
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      Expression::Advice(query) => {
        ...
        debug_struct.field("column_index", &query.column_index).field("rotation", &query.rotation);
        // Only show advice's phase if it's not in first phase.
      ...
      Expression::Challenge(challenge) => f.debug_tuple("Challenge").field(challenge).finish(),
...
impl<F: Field> Neg for Expression<F> {
  ...
  fn neg(self) -> Self::Output { Expression::Negated(Box::new(self)) }
}
...
impl<F: Field> Mul<F> for Expression<F> {
  ...
  fn mul(self, rhs: F) -> Expression<F> { Expression::Scaled(Box::new(self), rhs) }
}
...
impl<Col: Into<Column<Any>>> From<(Col, Rotation)> for VirtualCell {
  fn from((column, rotation): (Col, Rotation)) -> Self {
    VirtualCell { column: column.into(), rotation }
  }
}
...
impl<F: Field> From<Expression<F>> for Constraint<F> {
  fn from(poly: Expression<F>) -> Self { Constraint { name: "".to_string(), poly } }
}
...
impl<F: Field> From<Expression<F>> for Vec<Constraint<F>> {
  fn from(poly: Expression<F>) -> Self { vec![Constraint { name: "".to_string(), poly }] }
}
...
fn apply_selector_to_constraint<F: Field, C: Into<Constraint<F>>>(
  ...
  Constraint { name: constraint.name, poly: selector * constraint.poly }
}
...
impl<F: Field> Gate<F> {
  ...
  pub fn polynomials(&self) -> &[Expression<F>] { &self.polys }

  pub(crate) fn queried_selectors(&self) -> &[Selector] { &self.queried_selectors }

  pub(crate) fn queried_cells(&self) -> &[VirtualCell] { &self.queried_cells }
}
...
impl<'a, F: Field> std::fmt::Debug for PinnedGates<'a, F> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
    f.debug_list().entries(self.0.iter().flat_map(|gate| gate.polynomials().iter())).finish()
  }
}
...
impl<F: Field> ConstraintSystem<F> {
  ...
  pub fn shuffle<S: AsRef<str>>(
    ...
    self.shuffles.push(shuffle::Argument::new(name.as_ref(), shuffle_map));
  ...
  fn query_any_index(&mut self, column: Column<Any>, at: Rotation) -> usize {
    match column.column_type() {
      ...
      Any::Instance => self.query_instance_index(Column::<Instance>::try_from(column).unwrap(), at),
    }
  }
  ...
  pub(crate) fn get_any_query_index(&self, column: Column<Any>, at: Rotation) -> usize {
    match column.column_type() {
      ...
      Any::Fixed => self.get_fixed_query_index(Column::<Fixed>::try_from(column).unwrap(), at),
      Any::Instance =>
  ...
  pub fn set_minimum_degree(&mut self, degree: usize) { self.minimum_degree = Some(degree); }
  ...
  pub fn compress_selectors(mut self, selectors: Vec<Vec<bool>>) -> (Self, Vec<Vec<F>>) {
    ...
    for expr in self.shuffles.iter_mut().flat_map(|shuffle| {
      shuffle.input_expressions.iter_mut().chain(shuffle.shuffle_expressions.iter_mut())
    }) {
      replace_selectors(expr, &selector_replacements, true);
    }
  ...
  pub fn lookup_table_column(&mut self) -> TableColumn {
    TableColumn { inner: self.fixed_column() }
  }
  ...
  where
    ...
    self
      ...
      .insert(metadata::Column::from((Any::Fixed, column.inner().index)), annotation().into());
  }
  ...
  where
    ...
    self
      .general_column_annotations
      .insert(metadata::Column::from((col_any.column_type, col_any.index)), annotation().into());
  }

  /// Allocate a new fixed column
  pub fn fixed_column(&mut self) -> Column<Fixed> {
    let tmp = Column { index: self.num_fixed_columns, column_type: Fixed };
    self.num_fixed_columns += 1;
    tmp
  }

  /// Allocate a new advice column at `FirstPhase`
  pub fn advice_column(&mut self) -> Column<Advice> { self.advice_column_in(FirstPhase) }
  ...
  pub fn advice_column_in<P: Phase>(&mut self, phase: P) -> Column<Advice> {
    ...
    let tmp = Column { index: self.num_advice_columns, column_type: Advice { phase } };
  ...
  pub fn instance_column(&mut self) -> Column<Instance> {
    let tmp = Column { index: self.num_instance_columns, column_type: Instance };
  ...
  pub fn challenge_usable_after<P: Phase>(&mut self, phase: P) -> Challenge {
    let phase = phase.to_sealed();
    self.assert_phase_exists(phase, format!("Challenge usable after phase {:?}", phase).as_str());

    let tmp = Challenge { index: self.num_challenges, phase };
  ...
  pub(crate) fn phases(&self) -> impl Iterator<Item = sealed::Phase> {
    let max_phase = self.advice_column_phase.iter().max().map(|phase| phase.0).unwrap_or_default();
  ...
  pub fn degree(&self) -> usize {
    ...
    // for.
    degree =
      std::cmp::max(degree, self.lookups.iter().map(|l| l.required_degree()).max().unwrap_or(1));

    // The lookup argument also serves alongside the gates and must be accounted
    // for.
    degree =
      std::cmp::max(degree, self.shuffles.iter().map(|l| l.required_degree()).max().unwrap_or(1));
  ...
  pub fn blinding_factors(&self) -> usize {
    ...
    // - Each lookup argument has independent witness polynomials, and they are evaluated at most 2
    //   times.
  ...
  pub fn num_fixed_columns(&self) -> usize { self.num_fixed_columns }

  /// Returns number of advice columns
  pub fn num_advice_columns(&self) -> usize { self.num_advice_columns }

  /// Returns number of instance columns
  pub fn num_instance_columns(&self) -> usize { self.num_instance_columns }

  /// Returns number of selectors
  pub fn num_selectors(&self) -> usize { self.num_selectors }

  /// Returns number of challenges
  pub fn num_challenges(&self) -> usize { self.num_challenges }
  ...
  /// Returns gates
  pub fn gates(&self) -> &Vec<Gate<F>> { &self.gates }
  ...
  pub fn advice_queries(&self) -> &Vec<(Column<Advice>, Rotation)> { &self.advice_queries }

  /// Returns instance queries
  pub fn instance_queries(&self) -> &Vec<(Column<Instance>, Rotation)> { &self.instance_queries }

  /// Returns fixed queries
  pub fn fixed_queries(&self) -> &Vec<(Column<Fixed>, Rotation)> { &self.fixed_queries }

  /// Returns permutation argument
  pub fn permutation(&self) -> &permutation::Argument { &self.permutation }

  /// Returns lookup arguments
  pub fn lookups(&self) -> &Vec<lookup::Argument<F>> { &self.lookups }

  /// Returns shuffle arguments
  pub fn shuffles(&self) -> &Vec<shuffle::Argument<F>> { &self.shuffles }

  /// Returns constants
  pub fn constants(&self) -> &Vec<Column<Fixed>> { &self.constants }
}
...
impl<'a, F: Field> VirtualCells<'a, F> {
  fn new(meta: &'a mut ConstraintSystem<F>) -> Self {
    VirtualCells { meta, queried_selectors: vec![], queried_cells: vec![] }
  }
