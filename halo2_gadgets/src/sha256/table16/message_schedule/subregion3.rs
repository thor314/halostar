...
impl Subregion3Word {
  fn spread_a(&self) -> Value<[bool; 20]> { self.spread_a.value().map(|v| v.0) }

  fn spread_b(&self) -> Value<[bool; 14]> { self.b.value().map(|v| v.spread()) }

  fn spread_c(&self) -> Value<[bool; 4]> { self.c.value().map(|v| v.spread()) }

  fn spread_d(&self) -> Value<[bool; 26]> { self.spread_d.value().map(|v| v.0) }
