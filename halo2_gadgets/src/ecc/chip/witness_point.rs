...
impl Config {
  pub(super) fn configure(
    ...
    let config = Self { q_point: meta.selector(), q_point_non_id: meta.selector(), x, y };
  ...
  pub(super) fn point(
    ...
    self.assign_xy(value, offset, region).map(|(x, y)| EccPoint { x, y })
  }
  ...
  pub(super) fn point_non_id(
    ...
    self.assign_xy(value, offset, region).map(|(x, y)| NonIdentityEccPoint { x, y })
  }
}
