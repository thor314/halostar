...
impl Spec<Fp, 3, 2> for P128Pow5T3 {
  ...
  fn secure_mds() -> usize { unimplemented!() }

  fn constants() -> (Vec<[Fp; 3]>, Mds<Fp, 3>, Mds<Fp, 3>) {
    (super::fp::ROUND_CONSTANTS[..].to_vec(), super::fp::MDS, super::fp::MDS_INV)
  }
}
...
impl Spec<Fq, 3, 2> for P128Pow5T3 {
  ...
  fn sbox(val: Fq) -> Fq { val.pow_vartime([5]) }

  fn secure_mds() -> usize { unimplemented!() }

  fn constants() -> (Vec<[Fq; 3]>, Mds<Fq, 3>, Mds<Fq, 3>) {
    (super::fq::ROUND_CONSTANTS[..].to_vec(), super::fq::MDS, super::fq::MDS_INV)
  }
}
...
mod tests {
  ...
  use crate::poseidon::primitives::{generate_constants, permute, ConstantLength, Hash, Mds, Spec};
  ...
  impl<F: Field, const SECURE_MDS: usize> P128Pow5T3Gen<F, SECURE_MDS> {
    pub fn new() -> Self { P128Pow5T3Gen(PhantomData) }
  }
    ...
    for P128Pow5T3Gen<F, SECURE_MDS>
  {
    ...
    fn sbox(val: F) -> F { val.pow_vartime([5]) }

    fn secure_mds() -> usize { SECURE_MDS }

    fn constants() -> (Vec<[F; 3]>, Mds<F, 3>, Mds<F, 3>) { generate_constants::<_, Self, 3, 2>() }
  }
  ...
  fn verify_constants() {
    fn verify_constants_helper<F: FromUniformBytes<64> + Ord>(
      ...
      for (actual, expected) in
        round_constants.iter().flatten().zip(expected_round_constants.iter().flatten())
      {
      ...
      for (actual, expected) in mds_inv.iter().flatten().zip(expected_mds_inv.iter().flatten()) {
        assert_eq!(actual, expected);
      }
    }
  ...
  fn test_against_reference() {
    {
      ...
      // `generate_parameters_grain.sage 1 0 255 3 8 56
      // 0x40000000000000000000000000000000224698fc094cf91b992d30ed00000001`. The test vector
      // is generated by `sage poseidonperm_x5_pallas_3.sage --rust`
...

    {
      // <https://github.com/daira/pasta-hadeshash>, using parameters from
      // `generate_parameters_grain.sage 1 0 255 3 8 56
      // 0x40000000000000000000000000000000224698fc0994a8dd8c46eb2100000001`. The test vector
      // is generated by `sage poseidonperm_x5_vesta_3.sage --rust`
  ...
  fn hash_test_vectors() {
    for tv in crate::poseidon::primitives::test_vectors::fp::hash() {
      let message = [Fp::from_repr(tv.input[0]).unwrap(), Fp::from_repr(tv.input[1]).unwrap()];

      let result = Hash::<_, super::P128Pow5T3, ConstantLength<2>, 3, 2>::init().hash(message);

      assert_eq!(result.to_repr(), tv.output);
    }

    for tv in crate::poseidon::primitives::test_vectors::fq::hash() {
      let message = [Fq::from_repr(tv.input[0]).unwrap(), Fq::from_repr(tv.input[1]).unwrap()];
