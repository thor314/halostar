...
pub enum VerifyFailure {
  ...
  Lookup {
    ...
    /// - The input expressions do not correctly constrain a default value that exists in the table
    ///   when the lookup is not being used.
    /// - The input expressions use a column queried at a non-zero `Rotation`, and the lookup is
    ///   active on a row adjacent to an unrelated region.
    location:     FailureLocation,
  },
  ...
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      Self::CellNotAssigned { gate, region, gate_offset, column, offset } => {
        write!(
          f,
          "{} uses {} at offset {}, which requires cell in column {:?} at offset {} with \
           annotation {:?} to be assigned.",
          region,
          gate,
          gate_offset,
          column,
      ...
      Self::Shuffle { name, shuffle_index, location } => {
        write!(f, "Shuffle {}(index: {}) is not satisfied {}", name, shuffle_index, location)
      },
  ...
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      VerifyFailure::ConstraintNotSatisfied { constraint, location, cell_values } => {
        ...
        let ann_map = match location {
          FailureLocation::InRegion { region, offset: _ } => region.column_annotations.clone(),
          _ => None,
        };
        ...
        let debug = ConstraintCaseDebug {
          ...
          cell_values: cell_values
            .iter()
            .map(|(vc, value)| (DebugVirtualCell::from((vc, ann_map.as_ref())), value.clone()))
            .collect(),
        };
...
fn render_lookup<F: Field>(
  ...
  fn cell_value<'a, F: Field, Q: Into<AnyQuery> + Copy>(
    ...
    move |query| {
      ...
      Some((((column_type, column_index).into(), rotation.0).into(), match load(query) {
...
fn render_shuffle<F: Field>(
  ...
  for (i, input) in shuffle.input_expressions.iter().enumerate() {
    ...
    eprintln!("    Sh{} = {}", i, emitter::expression_to_string(input, &layout));
    eprintln!("    ^");
...
impl VerifyFailure {
  ...
  pub(super) fn emit<F: Field>(&self, prover: &MockProver<F>) {
    match self {
      Self::CellNotAssigned { gate, region, gate_offset, column, offset } =>
        render_cell_not_assigned(&prover.cs.gates, gate, region, *gate_offset, *column, *offset),
      Self::ConstraintNotSatisfied { constraint, location, cell_values } =>
