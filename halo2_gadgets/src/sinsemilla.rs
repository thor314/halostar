...
where SinsemillaChip: SinsemillaInstructions<C, K, MAX_WORDS> + Clone + Debug + Eq {
  chip:  SinsemillaChip,
...
where SinsemillaChip: SinsemillaInstructions<C, K, MAX_WORDS> + Clone + Debug + Eq
{
  fn from_bitstring(
    ...
    let pieces: Result<Vec<_>, _> = bitstring
      ...
      .enumerate()
      .map(|(i, piece)| -> Result<MessagePiece<C, SinsemillaChip, K, MAX_WORDS>, Error> {
  ...
  pub fn from_pieces(
    ...
    Self { chip, inner: pieces.into_iter().map(|piece| piece.inner).collect::<Vec<_>>().into() }
  }
}
...
where SinsemillaChip: SinsemillaInstructions<C, K, MAX_WORDS> + Clone + Debug + Eq
{
  ...
  pub fn inner(&self) -> SinsemillaChip::MessagePiece { self.inner.clone() }
}
...
where SinsemillaChip: SinsemillaInstructions<C, K, MAX_WORDS> + Clone + Debug + Eq
{
  ...
  pub fn from_subpieces(
    ...
    let (field_elem, total_bits) =
      subpieces.into_iter().fold((Value::known(C::Base::ZERO), 0), |(acc, bits), subpiece| {
        assert!(bits < 64);
        let subpiece_shifted = subpiece.inner().value().map(|v| C::Base::from(1 << bits) * v);
...
pub(crate) mod tests {
  ...
  impl CommitDomains<pallas::Affine, TestFixedBases, TestHashDomain> for TestCommitDomain {
    fn r(&self) -> FullWidth { FullWidth::from_parts(*R, &R_ZS_AND_US) }

    fn hash_domain(&self) -> TestHashDomain { TestHashDomain }
  }
  ...
  impl Circuit<pallas::Base> for MyCircuit {
    ...
    fn synthesize(
      ...
      // It is not an implementation of the Orchard protocol spec.
      {
        ...
        let l =
          MessagePiece::from_bitstring(chip1.clone(), layouter.namespace(|| "l"), &l_bitstring)?;

        // Left leaf
        let left_bitstring: Vec<Value<bool>> =
          (0..250).map(|_| Value::known(rand::random::<bool>())).collect();
        ...
        // Right leaf
        let right_bitstring: Vec<Value<bool>> =
          (0..250).map(|_| Value::known(rand::random::<bool>())).collect();
        ...
        let expected_parent = {
          let expected_parent =
            l_bitstring.zip(left_bitstring.zip(right_bitstring)).map(|(l, (left, right))| {
              let merkle_crh = sinsemilla::HashDomain::from_Q((*Q).into());
              let point = merkle_crh.hash_to_point(l.into_iter().chain(left).chain(right)).unwrap();
              point.to_affine()
            });
        ...
        parent
          .constrain_equal(layouter.namespace(|| "parent == expected parent"), &expected_parent)?;
      }

      {
        ...
        let message: Vec<Value<bool>> =
          (0..500).map(|_| Value::known(rand::random::<bool>())).collect();

        let (result, _) = {
          let r =
            ScalarFixed::new(ecc_chip.clone(), layouter.namespace(|| "r"), Value::known(r_val))?;
        ...
        result.constrain_equal(layouter.namespace(|| "result == expected result"), &expected_result)
      }
    }
  }
  ...
  fn print_sinsemilla_chip() {
    ...
    let root = BitMapBackend::new("sinsemilla-hash-layout.png", (1024, 7680)).into_drawing_area();
    root.fill(&WHITE).unwrap();
    let root = root.titled("SinsemillaHash", ("sans-serif", 60)).unwrap();

    let circuit = MyCircuit {};
    halo2_proofs::dev::CircuitLayout::default().render(11, &circuit, &root).unwrap();
  }
}
