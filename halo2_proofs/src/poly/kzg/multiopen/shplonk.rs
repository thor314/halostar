...
impl<F: Field, T: PartialEq + Clone> Commitment<F, T> {
  fn get(&self) -> T { self.0 .0.clone() }

  fn evals(&self) -> Vec<F> { self.0 .1.clone() }
}
...
where I: IntoIterator<Item = Q> + Clone {
  ...
  for query in queries.iter() {
      ...
      .find(|(commitment, _)| *commitment == query.get_commitment())
    {
      ...
      commitment_rotation_set_map
        .push((query.get_commitment(), BTreeSet::from_iter(std::iter::once(rotation))));
    };
  }
  ...
  // {r_2, r_3, r_4} : [C_2, C_3],
  // ...
  // NOTE: we want to make the order of the collection of rotation sets independent of the opening
  // points, to ease the verifier computation
  ...
  let rotation_sets = rotation_set_commitment_map
    ...
    .map(|(rotations, commitments)| {
      ...
      let commitments: Vec<Commitment<F, Q::Commitment>> = commitments
        ...
        .map(|commitment| {
          let evals: Vec<F> =
            rotations_vec.par_iter().map(|&&rotation| get_eval(commitment, rotation)).collect();
...
mod proptests {
  ...
  impl super::Query<Fp> for MyQuery<Fp> {
    ...
    fn get_point(&self) -> Fp { self.point }

    fn get_eval(&self) -> Self::Eval { self.eval }

    fn get_commitment(&self) -> Self::Commitment { self.commitment }
  }
