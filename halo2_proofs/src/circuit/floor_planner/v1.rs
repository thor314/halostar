...
///
/// - No column optimizations are performed. Circuit configuration is left entirely to the circuit
///   designer.
...
/// - Regions are laid out using a greedy first-fit strategy, after sorting regions by their "advice
///   area" (number of advice columns * rows).
#[derive(Debug)]
pub struct V1;
...
impl<'a, F: Field, CS: Assignment<F> + SyncDeps> V1Plan<'a, F, CS> {
  ...
  pub fn new(cs: &'a mut CS) -> Result<Self, Error> {
    let ret = V1Plan { cs, regions: vec![], constants: vec![], table_columns: vec![] };
    Ok(ret)
  }
}
...
impl FloorPlanner for V1 {
  fn synthesize<F: Field, CS: Assignment<F> + SyncDeps, C: Circuit<F>>(
    ...
    let mut measure = MeasurementPass::new();
    {
      ...
      circuit.without_witnesses().synthesize(config.clone(), V1Pass::<_, CS>::measure(pass))?;
    }
    ...
    let first_unassigned_row =
      column_allocations.values().map(|a| a.unbounded_interval_start()).max().unwrap_or(0);

    // - Position the constants within those rows.
    let fixed_allocations: Vec<_> = constants
      .into_iter()
      .map(|c| {
        (c, column_allocations.get(&Column::<Any>::from(c).into()).cloned().unwrap_or_default())
      })
      .collect();
...
impl<'p, 'a, F: Field, CS: Assignment<F> + 'a> V1Pass<'p, 'a, F, CS> {
  fn measure(pass: &'p mut MeasurementPass) -> Self { V1Pass(Pass::Measurement(pass)) }

  fn assign(pass: &'p mut AssignmentPass<'p, 'a, F, CS>) -> Self { V1Pass(Pass::Assignment(pass)) }
}
...
impl MeasurementPass {
  fn new() -> Self { MeasurementPass { regions: vec![] } }
...
impl<'p, 'a, F: Field, CS: Assignment<F> + SyncDeps> AssignmentPass<'p, 'a, F, CS> {
  fn new(plan: &'p mut V1Plan<'a, F, CS>) -> Self { AssignmentPass { plan, region_index: 0 } }
  ...
  where
    ...
    let first_unused = {
      match default_and_assigned
        .values()
        .map(|(_, assigned)| if assigned.iter().all(|b| *b) { Some(assigned.len()) } else { None })
...
impl<'r, 'a, F: Field, CS: Assignment<F> + SyncDeps> RegionLayouter<F> for V1Region<'r, 'a, F, CS> {
  ...
  fn assign_advice_from_constant<'v>(
    ...
    let advice = self.assign_advice(annotation, column, offset, &mut || Value::known(constant))?;
