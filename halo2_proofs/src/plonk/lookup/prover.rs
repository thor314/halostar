...
impl<F: WithSmallOrderMulGroup<3>> Argument<F> {
  ...
  /// - constructs A_compressed = \theta^{m-1} A_0 + theta^{m-2} A_1 + ... + \theta A_{m-2} +
  ///   A_{m-1} and S_compressed = \theta^{m-1} S_0 + theta^{m-2} S_1 + ... + \theta S_{m-2} +
  ///   S_{m-1},
  /// - permutes A_compressed and S_compressed using permute_expression_pair() helper, obtaining A'
  ///   and S', and
  /// - constructs Permuted<C> struct using permuted_input_value = A', and permuted_table_expression
  ///   = S'.
...
impl<C: CurveAffine> Permuted<C> {
  ...
  >(
    ...
    //
    // Numerator: (\theta^{m-1} a_0(\omega^i) + \theta^{m-2} a_1(\omega^i) + ... + \theta
    ...
    // (\theta^{m-1} a_0(\omega^i) + \theta^{m-2} a_1(\omega^i) + ... + \theta a_{m-2}(\omega^i) +
    // a_{m-1}(\omega^i) + \beta)
    // * (\theta^{m-1} s_0(\omega^i) + \theta^{m-2} s_1(\omega^i) + ... + \theta s_{m-2}(\omega^i) +
    //   s_{m-1}(\omega^i) + \gamma)
...
/// - the first row in a sequence of like values in A' is the row that has the corresponding value
///   in S'.
