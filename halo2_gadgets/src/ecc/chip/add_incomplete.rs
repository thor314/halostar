...
impl Config {
  pub(super) fn configure(
    ...
    let config = Self { q_add_incomplete: meta.selector(), x_p, y_p, x_qr, y_qr };
  ...
  pub(crate) fn advice_columns(&self) -> HashSet<Column<Advice>> {
    [self.x_p, self.y_p, self.x_qr, self.y_qr].into_iter().collect()
  }
  ...
  pub(super) fn assign_region(
    ...
    x_p.zip(y_p).zip(x_q).zip(y_q).error_if_known_and(|(((x_p, y_p), x_q), y_q)| {
    ...
    let r = x_p.zip(y_p).zip(x_q).zip(y_q).map(|(((x_p, y_p), x_q), y_q)| {
      {
        // λ = (y_q - y_p)/(x_q - x_p)
    ...
    let result = NonIdentityEccPoint { x: x_r_var, y: y_r_var };
