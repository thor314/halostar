...
impl fmt::Debug for Estimator {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "Estimator") }
}
...
impl Estimator {
  fn random(k: usize) -> Self {
    ...
    Estimator {
      multiexp_scalars: (0..max_size).map(|_| pallas::Scalar::random(&mut rng)).collect(),
...
struct CostOptions {
  ...
  #[options(help = "A fixed column with the given rotations. May be repeated.", meta = "R[,R..]")]
  fixed: Vec<Poly>,
  ...
  #[options(
    help = "A lookup over N columns with max input degree I and max table degree T. May be \
            repeated.",
    meta = "N,I,T"
  )]
  lookup: Vec<Lookup>,
...
impl FromStr for Poly {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    let mut rotations: Vec<isize> = s.split(',').map(|r| r.parse()).collect::<Result<_, _>>()?;
...
impl FromStr for Lookup {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    ...
    Ok(Lookup { _columns, input_deg, table_deg })
  }
}
...
impl FromStr for Permutation {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> { Ok(Permutation { columns: s.parse()? }) }
}

impl Permutation {
  fn required_degree(&self) -> usize { cmp::max(self.columns + 1, 2) }
  ...
  fn queries(&self) -> impl Iterator<Item = Poly> {
    ...
    iter::empty().chain(Some(product)).chain(iter::repeat(poly).take(self.columns))
  }
}
...
impl Circuit {
  fn proof_size(&self) -> usize {
    ...
    let plonk = size(1, 0) * self.advice_columns
      ...
      + self.permutations.iter().map(|p| size(1, 2 + p.columns)).sum::<usize>();

    // Vanishing argument:
    // - (max_deg - 1) * 32 bytes (commitments) + (max_deg - 1) * 32 bytes (h_evals) for quotient
    //   polynomial
...
fn main() {
  ...
  println!("Verification: at least {}ms", c.verification_time().as_micros() as f64 / 1_000f64);
}
