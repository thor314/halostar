...
impl<F: PrimeFieldBits> std::ops::Deref for RunningSum<F> {
  ...
  fn deref(&self) -> &Vec<AssignedCell<F, F>> { &self.0 }
}
...
impl<F: PrimeFieldBits> RangeConstrained<F, AssignedCell<F, F>> {
  ...
  pub fn witness_short<const K: usize>(
    ...
    lookup_config
      .witness_short_check(layouter, value.map(|value| bitrange_subset(value, bitrange)), num_bits)
      .map(|inner| Self { inner, num_bits, _phantom: PhantomData })
  }
}
...
impl<F: PrimeFieldBits, const K: usize> LookupRangeCheckConfig<F, K> {
  ...
  pub fn configure(
    ...
    meta.create_gate("Short lookup bitshift", |meta| {
      ...
      Constraints::with_selector(q_bitshift, Some(word * two_pow_k * inv_two_pow_s - shifted_word))
    });
  ...
  pub fn witness_check(
    ...
    layouter.assign_region(
      || "Witness element",
      |mut region| {
        let z_0 = region.assign_advice(|| "Witness element", self.running_sum, 0, || value)?;
        self.range_check(&mut region, z_0, num_words, strict)
      },
    )
  }
  ...
  fn range_check(
    ...
    for (idx, word) in words.iter().enumerate() {
      ...
      z = {
        let z_val = z.value().zip(*word).map(|(z, word)| (*z - word) * inv_two_pow_k);

        // Assign z_next
        region.assign_advice(|| format!("z_{:?}", idx + 1), self.running_sum, idx + 1, || z_val)?
      };
      zs.push(z.clone());
    }
  ...
  pub fn copy_short_check(
    ...
    layouter.assign_region(
      ...
      |mut region| {
        ...
        let element = element.copy_advice(|| "element", &mut region, self.running_sum, 0)?;
...
mod tests {
  ...
  use super::{super::lebs2ip, LookupRangeCheckConfig};
  ...
  fn lookup_range_check() {
    ...
    {
      let circuit: MyCircuit<pallas::Base> = MyCircuit { num_words: 6, _marker: PhantomData };
  ...
  fn short_range_check() {
    ...
    impl<F: PrimeFieldBits> Circuit<F> for MyCircuit<F> {
      ...
      fn without_witnesses(&self) -> Self {
        MyCircuit { element: Value::unknown(), num_bits: self.num_bits }
      }
    ...
    // Edge case: zero bits
    {
      let circuit: MyCircuit<pallas::Base> =
        MyCircuit { element: Value::known(pallas::Base::zero()), num_bits: 0 };
      let prover = MockProver::<pallas::Base>::run(11, &circuit, vec![]).unwrap();
      assert_eq!(prover.verify(), Ok(()));
    }
    ...
    {
      let circuit: MyCircuit<pallas::Base> =
        MyCircuit { element: Value::known(pallas::Base::from((1 << K) - 1)), num_bits: K };
      let prover = MockProver::<pallas::Base>::run(11, &circuit, vec![]).unwrap();
      assert_eq!(prover.verify(), Ok(()));
    }
    ...
    {
      let circuit: MyCircuit<pallas::Base> =
        MyCircuit { element: Value::known(pallas::Base::from((1 << 6) - 1)), num_bits: 6 };
      let prover = MockProver::<pallas::Base>::run(11, &circuit, vec![]).unwrap();
      assert_eq!(prover.verify(), Ok(()));
    }
    ...
    {
      let circuit: MyCircuit<pallas::Base> =
        MyCircuit { element: Value::known(pallas::Base::from(1 << 6)), num_bits: 6 };
      let prover = MockProver::<pallas::Base>::run(11, &circuit, vec![]).unwrap();
      assert_eq!(
        prover.verify(),
    ...
    {
      let circuit: MyCircuit<pallas::Base> =
        MyCircuit { element: Value::known(pallas::Base::from(1 << K)), num_bits: 6 };
      let prover = MockProver::<pallas::Base>::run(11, &circuit, vec![]).unwrap();
      assert_eq!(
        prover.verify(),
        Err(vec![
    ...
    // num_bits
    {
      ...
      let element = shifted * pallas::Base::from(1 << (K as u64 - num_bits)).invert().unwrap();
      let circuit: MyCircuit<pallas::Base> =
        MyCircuit { element: Value::known(element), num_bits: num_bits as usize };
      let prover = MockProver::<pallas::Base>::run(11, &circuit, vec![]).unwrap();
      assert_eq!(
        prover.verify(),
        Err(vec![VerifyFailure::Lookup {
