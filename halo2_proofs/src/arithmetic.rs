...
pub fn eval_polynomial<F: Field>(poly: &[F], point: F) -> F {
  ...
  if n * 2 < num_threads {
    ...
    multicore::scope(|scope| {
      for (chunk_idx, (out, poly)) in parts.chunks_mut(1).zip(poly.chunks(chunk_size)).enumerate() {
        scope.spawn(move |_| {
...
pub fn lagrange_interpolate<F: Field>(points: &[F], evals: &[F]) -> Vec<F> {
  ...
  if points.len() == 1 {
    ...
    for (j, (denoms, eval)) in denoms.into_iter().zip(evals.iter()).enumerate() {
      ...
      tmp.push(F::ONE);
      for (x_k, denom) in
        points.iter().enumerate().filter(|&(k, _)| k != j).map(|a| a.1).zip(denoms.into_iter())
      {
...
#[cfg(test)] use rand_core::OsRng;

#[cfg(test)] use crate::halo2curves::pasta::Fp;
