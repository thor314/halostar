...
> {
  ...
  fn xor_upper_sigma(&self) -> Value<[bool; 64]> {
    self.spread_a().zip(self.spread_b()).zip(self.spread_c()).zip(self.spread_d()).map(
      |(((a, b), c), d)| {
        let xor_0 =
          b.iter().chain(c.iter()).chain(d.iter()).chain(a.iter()).copied().collect::<Vec<_>>();
        let xor_1 =
          c.iter().chain(d.iter()).chain(a.iter()).chain(b.iter()).copied().collect::<Vec<_>>();
        let xor_2 =
          d.iter().chain(a.iter()).chain(b.iter()).chain(c.iter()).copied().collect::<Vec<_>>();
...
impl AbcdVar {
  fn a_range() -> Range<usize> { 0..2 }

  fn b_range() -> Range<usize> { 2..13 }

  fn c_lo_range() -> Range<usize> { 13..16 }

  fn c_mid_range() -> Range<usize> { 16..19 }

  fn c_hi_range() -> Range<usize> { 19..22 }

  fn d_range() -> Range<usize> { 22..32 }
...
impl UpperSigmaVar<4, 22, 18, 20> for AbcdVar {
  fn spread_a(&self) -> Value<[bool; 4]> { self.a.spread.value().map(|v| v.0) }

  fn spread_b(&self) -> Value<[bool; 22]> { self.b.spread.value().map(|v| v.0) }

  fn spread_c(&self) -> Value<[bool; 18]> {
    self.c_lo.spread.value().zip(self.c_mid.spread.value()).zip(self.c_hi.spread.value()).map(
      |((c_lo, c_mid), c_hi)| {
        c_lo
          .iter()
          .chain(c_mid.iter())
  ...
  fn spread_d(&self) -> Value<[bool; 20]> { self.d.spread.value().map(|v| v.0) }
}
...
impl EfghVar {
  fn a_lo_range() -> Range<usize> { 0..3 }

  fn a_hi_range() -> Range<usize> { 3..6 }

  fn b_lo_range() -> Range<usize> { 6..8 }

  fn b_hi_range() -> Range<usize> { 8..11 }

  fn c_range() -> Range<usize> { 11..25 }

  fn d_range() -> Range<usize> { 25..32 }
...
impl UpperSigmaVar<12, 10, 28, 14> for EfghVar {
  fn spread_a(&self) -> Value<[bool; 12]> {
    self.a_lo.spread.value().zip(self.a_hi.spread.value()).map(|(a_lo, a_hi)| {
      a_lo.iter().chain(a_hi.iter()).copied().collect::<Vec<_>>().try_into().unwrap()
    })
  }

  fn spread_b(&self) -> Value<[bool; 10]> {
    self.b_lo.spread.value().zip(self.b_hi.spread.value()).map(|(b_lo, b_hi)| {
      b_lo.iter().chain(b_hi.iter()).copied().collect::<Vec<_>>().try_into().unwrap()
    })
  }

  fn spread_c(&self) -> Value<[bool; 28]> { self.c.spread.value().map(|v| v.0) }

  fn spread_d(&self) -> Value<[bool; 14]> { self.d.spread.value().map(|v| v.0) }
}
...
impl From<(AssignedBits<16>, AssignedBits<16>)> for RoundWordDense {
  fn from(halves: (AssignedBits<16>, AssignedBits<16>)) -> Self { Self(halves.0, halves.1) }
}

impl RoundWordDense {
  pub fn value(&self) -> Value<u32> {
    self.0.value_u16().zip(self.1.value_u16()).map(|(lo, hi)| lo as u32 + (1 << 16) * hi as u32)
  }
}
...
impl From<(AssignedBits<32>, AssignedBits<32>)> for RoundWordSpread {
  fn from(halves: (AssignedBits<32>, AssignedBits<32>)) -> Self { Self(halves.0, halves.1) }
}

impl RoundWordSpread {
  pub fn value(&self) -> Value<u64> {
    self.0.value_u32().zip(self.1.value_u32()).map(|(lo, hi)| lo as u64 + (1 << 32) * hi as u64)
  }
}
...
impl RoundWordA {
  pub fn new(
    ...
    RoundWordA { pieces: Some(pieces), dense_halves, spread_halves: Some(spread_halves) }
  }

  pub fn new_dense(dense_halves: RoundWordDense) -> Self {
    RoundWordA { pieces: None, dense_halves, spread_halves: None }
  }
}
...
impl RoundWordE {
  pub fn new(
    ...
    RoundWordE { pieces: Some(pieces), dense_halves, spread_halves: Some(spread_halves) }
  }

  pub fn new_dense(dense_halves: RoundWordDense) -> Self {
    RoundWordE { pieces: None, dense_halves, spread_halves: None }
  }
}
...
impl RoundWord {
  pub fn new(dense_halves: RoundWordDense, spread_halves: RoundWordSpread) -> Self {
    RoundWord { dense_halves, spread_halves }
  }
}
...
impl State {
  ...
  pub fn empty_state() -> Self {
    State { a: None, b: None, c: None, d: None, e: None, f: None, g: None, h: None }
  }
}
  ...
  pub(super) fn configure(
    ...
    meta.create_gate("s_digest", |meta| {
      ...
      CompressionGate::s_digest(
        s_digest, lo_0, hi_0, word_0, lo_1, hi_1, word_1, lo_2, hi_2, word_2, lo_3, hi_3, word_3,
      )
    });
...
mod tests {
  ...
  fn compress() {
    ...
    impl Circuit<pallas::Base> for MyCircuit {
      ...
      fn without_witnesses(&self) -> Self { MyCircuit {} }
