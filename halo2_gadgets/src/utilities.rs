...
impl<F: Field> FieldValue<F> for Value<F> {
  fn value(&self) -> Value<&F> { self.as_ref() }
}

impl<F: Field> FieldValue<F> for AssignedCell<F, F> {
  fn value(&self) -> Value<&F> { self.value() }
}
...
impl<F: Field> Var<F> for AssignedCell<F, F> {
  fn cell(&self) -> Cell { self.cell() }

  fn value(&self) -> Value<F> { self.value().cloned() }
}
...
pub trait UtilitiesInstructions<F: Field> {
  ...
  fn load_private(
    ...
    layouter.assign_region(
      || "load private",
      |mut region| {
        region.assign_advice(|| "load private", column, 0, || value).map(Self::Var::from)
      },
    )
  }
}
...
impl<F: Field> RangeConstrained<F, AssignedCell<F, F>> {
  ...
  pub fn unsound_unchecked(cell: AssignedCell<F, F>, num_bits: usize) -> Self {
    Self { inner: cell, num_bits, _phantom: PhantomData }
  }
...
pub fn bool_check<F: PrimeField>(value: Expression<F>) -> Expression<F> { range_check(value, 2) }
...
pub fn bitrange_subset<F: PrimeFieldBits>(field_elem: &F, bitrange: Range<usize>) -> F {
  ...
  field_elem
    ...
    .rev()
    .fold(F::ZERO, |acc, bit| if bit { acc.double() + F::ONE } else { acc.double() })
}
...
mod tests {
  ...
  fn test_range_check() {
    ...
    impl<const RANGE: usize> Circuit<pallas::Base> for MyCircuit<RANGE> {
      ...
      fn without_witnesses(&self) -> Self { MyCircuit(self.0) }
  ...
  fn test_bitrange_subset() {
    ...
    let decompose = |field_elem: pallas::Base, ranges: &[Range<usize>]| {
      ...
      let subsets =
        ranges.iter().map(|range| bitrange_subset(&field_elem, range.clone())).collect::<Vec<_>>();
    ...
    decompose(pallas::Base::random(rng), &[0..50, 50..100, 100..150, 150..200, 200..255]);
  }
  ...
  fn lebs2ip_round_trip() {
    ...
    assert_eq!(lebs2ip::<64>(&i2lebsp(0xFFFFFFFFFFFFFFFF)), 0xFFFFFFFFFFFFFFFF);
  }
