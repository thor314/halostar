...
pub use floor_planner::single_pass::{SimpleFloorPlanner, SimpleTableLayouter};
...
impl From<usize> for RegionIndex {
  fn from(idx: usize) -> RegionIndex { RegionIndex(idx) }
}
...
impl From<usize> for RegionStart {
  fn from(idx: usize) -> RegionStart { RegionStart(idx) }
}
...
where for<'v> Assigned<F>: From<&'v V>
{
  ...
  pub fn value_field(&self) -> Value<Assigned<F>> { self.value.to_field() }
}
...
where for<'v> Assigned<F>: From<&'v V>
{
  ...
  where
    ...
    AR: Into<String>,
  {
    let assigned_cell = region.assign_advice(annotation, column, offset, || self.value.clone())?;
...
impl<'r, F: Field> From<&'r mut dyn layouter::RegionLayouter<F>> for Region<'r, F> {
  fn from(region: &'r mut dyn layouter::RegionLayouter<F>) -> Self { Region { region } }
}
...
impl<'r, F: Field> Region<'r, F> {
    ...
    AR: Into<String>,
  {
    ...
    let cell = self.region.assign_advice(&|| annotation().into(), column, offset, &mut || {
      let v = to();
  ...
  {
    let mut value = Value::unknown();
    let cell = self.region.assign_fixed(&|| annotation().into(), column, offset, &mut || {
      let v = to();
      let value_f = v.to_field();
      value = v;
      value_f
...
impl<'r, F: Field> From<&'r mut dyn layouter::TableLayouter<F>> for Table<'r, F> {
  fn from(table: &'r mut dyn layouter::TableLayouter<F>) -> Self { Table { table } }
}
...
impl<'a, F: Field, L: Layouter<F> + 'a> Layouter<F> for NamespacedLayouter<'a, F, L> {
  ...
  fn get_challenge(&self, challenge: Challenge) -> Value<F> { self.0.get_challenge(challenge) }
