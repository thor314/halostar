...
fn plonk_api() {
  ...
  impl<FF: Field> StandardPlonk<FF> {
    fn new(config: PlonkConfig) -> Self { StandardPlonk { config, _marker: PhantomData } }
  }
      ...
      layouter.assign_table(
        ...
        |mut table| {
          for (index, &value) in values.iter().enumerate() {
            table.assign_cell(|| "table col", self.config.sl, index, || Value::known(value))?;
          }
          Ok(())
        },
      )?;
  ...
  where Scheme::Scalar: FromUniformBytes<64> + WithSmallOrderMulGroup<3> {
    let (_, _, lookup_table) = common!(Scheme);
    let empty_circuit: MyCircuit<Scheme::Scalar> = MyCircuit { a: Value::unknown(), lookup_table };
  ...
  fn test_plonk_api_gwc() {
    ...
    let proof =
      create_proof::<_, ProverGWC<_>, _, _, Blake2bWrite<_, _, Challenge255<_>>>(rng, &params, &pk);
  ...
  fn test_plonk_api_ipa() {
    ...
    let proof =
      create_proof::<_, ProverIPA<_>, _, _, Blake2bWrite<_, _, Challenge255<_>>>(rng, &params, &pk);
