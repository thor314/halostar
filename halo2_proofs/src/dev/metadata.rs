...
impl Column {
  ...
  pub fn column_type(&self) -> Any { self.column_type }
...
impl From<(Any, usize)> for Column {
  fn from((column_type, index): (Any, usize)) -> Self { Column { column_type, index } }
}
...
/// Helper structure used to be able to inject Column annotations inside a `Display` or `Debug`
/// call.
#[derive(Clone, Debug)]
...
impl<S: AsRef<str>> From<(usize, S)> for Gate {
  fn from((index, name): (usize, S)) -> Self { Gate { index, name: name.as_ref().to_string() } }
}
...
impl fmt::Display for Constraint {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(
      ...
      "Constraint {}{} in gate {} ('{}')",
      self.index,
      if self.name.is_empty() { String::new() } else { format!(" ('{}')", self.name.as_str()) },
      self.gate.index,
      self.gate.name,
    )
  }
...
impl PartialEq for Region {
  fn eq(&self, other: &Self) -> bool { self.index == other.index && self.name == other.name }
}
