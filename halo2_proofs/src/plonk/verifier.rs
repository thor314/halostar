...
use crate::{
  arithmetic::{compute_inner_product, CurveAffine},
  poly::{
    commitment::{Blind, CommitmentScheme, Params, Verifier, MSM},
    Guard, VerificationStrategy, VerifierQuery,
  },
  transcript::{read_n_points, read_n_scalars, EncodedChallenge, TranscriptRead},
};

#[cfg(feature = "batch")] mod batch;
#[cfg(feature = "batch")] pub use batch::BatchVerifier;
  ...
  Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
{
  ...
  let instance_evals = if V::QUERY_INSTANCE {
    ...
    let max_instance_len = instances
      ...
      .unwrap_or_default();
    let l_i_s =
      &vk.domain.l_i_range(*x, xn, -max_rotation..max_instance_len as i32 + min_rotation.abs());
    instances
      .iter()
      .map(|instances| {
        vk.cs
  ...
  let lookups_evaluated = lookups_committed
    ...
    .map(|lookups| -> Result<Vec<_>, _> {
      lookups.into_iter().map(|lookup| lookup.evaluate(transcript)).collect::<Result<Vec<_>, _>>()
    })
    .collect::<Result<Vec<_>, _>>()?;
  ...
  let vanishing = {
    ...
    let l_blind: Scheme::Scalar =
      l_evals[1..(1 + blinding_factors)].iter().fold(Scheme::Scalar::ZERO, |acc, eval| acc + eval);
    ...
    let expressions = advice_evals
      ...
      .flat_map(|((((advice_evals, instance_evals), permutation), lookups), shuffles)| {
