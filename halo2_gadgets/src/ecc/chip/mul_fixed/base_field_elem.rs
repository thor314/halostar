...
use super::{
  super::{EccBaseFieldElemFixed, EccPoint, FixedPoints, NUM_WINDOWS, T_P},
  H_BASE,
};
use crate::{
  sinsemilla::primitives as sinsemilla,
  utilities::{
    bitrange_subset, bool_check, lookup_range_check::LookupRangeCheckConfig, range_check,
  },
};
...
impl<Fixed: FixedPoints<pallas::Affine>> Config<Fixed> {
  pub(crate) fn configure(
    ...
    let config =
      Self { q_mul_fixed_base_field: meta.selector(), canon_advices, lookup_config, super_config };
    ...
    meta.create_gate("Canonicity checks", |meta| {
      ...
      let decomposition_checks = {
        ...
        let z_84_alpha_check =
          z_84_alpha.clone() - (alpha_1.clone() + alpha_2.clone() * pallas::Base::from(1 << 2));
      ...
      //          - α_2 = 1 => 0 ≤ α_0 + 2^130 - t_p < 2^130 => 13 ten-bit lookups of α_0 + 2^130 -
      //            t_p => z_13_alpha_0_prime = 0
      //
      let canon_checks = {
        // alpha_0_hi_120 = z_44 - 2^120 z_84
        let alpha_0_hi_120 = {
          let two_pow_120 = Expression::Constant(pallas::Base::from_u128(1 << 60).square());
        ...
        std::iter::empty()
          ...
          .chain(Some(("MSB = 1 => alpha_0_hi_120 = 0", alpha_2.clone() * alpha_0_hi_120)))
          .chain(Some(("MSB = 1 => a_43 = 0 or 1", alpha_2.clone() * bool_check(a_43))))
          .chain(Some(("MSB = 1 => z_13_alpha_0_prime = 0", alpha_2 * z_13_alpha_0_prime)))
      };
    ...
    <Fixed as FixedPoints<pallas::Affine>>::Base: super::super::FixedPoint<pallas::Affine>,
  {
    ...
    // Check that the correct multiple is obtained.
    {
      ...
      real_mul.zip(result).assert_if_known(|(real_mul, result)| &real_mul.to_affine() == result);
    }
    ...
    //      - α_2 = 1 => α_1 = 0, and
    //      - α_2 = 1 => α_0 < t_p. To enforce this:
    //      - α_2 = 1 => 0 ≤ α_0 < 2^130 => 13 ten-bit lookups of α_0
    //      - α_2 = 1 => 0 ≤ α_0 + 2^130 - t_p < 2^130 => 13 ten-bit lookups of α_0 + 2^130 - t_p =>
    ...
    let alpha_0 = alpha.value().zip(z_84_alpha.value()).map(|(alpha, z_84_alpha)| {
      let two_pow_252 = pallas::Base::from_u128(1 << 126).square();
    ...
    layouter.assign_region(
      ...
      |mut region| {
        ...
        // Offset 1
        {
          ...
          region.assign_advice(|| "α_2 = α[254]", self.canon_advices[2], offset, || alpha_2)?;
        }
...
pub mod tests {
  ...
  fn test_single_base(
    ...
    // There is a single canonical sequence of window values for which a doubling occurs on the last
    // step:
    // 1333333333333333333333333333333333333333333333333333333333333333333333333333333333334 in
    // octal. (There is another *non-canonical* sequence
    // 5333333333333333333333333333333333333333332711161673731021062440252244051273333333333 in
    // octal.)
    {
      let h = pallas::Base::from(H as u64);
      let scalar_fixed =
        "1333333333333333333333333333333333333333333333333333333333333333333333333333333333334"
          .chars()
    ...
    // on the last step.
    {
      ...
      let result = {
        let scalar_fixed =
          chip.load_private(layouter.namespace(|| "zero"), column, Value::known(scalar_fixed))?;
        base.mul(layouter.namespace(|| "mul by zero"), scalar_fixed)?
      };
      result.inner().is_identity().assert_if_known(|is_identity| *is_identity);
    }
    ...
    // [-1]B is the largest base field element
    {
      ...
      let result = {
        let scalar_fixed =
          chip.load_private(layouter.namespace(|| "-1"), column, Value::known(scalar_fixed))?;
