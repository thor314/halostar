...
where C::Scalar: SerdePrimeField + FromUniformBytes<64>
{
  ...
  /// Checks that field elements are less than modulus, and then checks that the point is on the
  /// curve.
  /// - `RawBytesUnchecked`: Reads an uncompressed curve element with coordinates in Montgomery
  ///   form;
  ...
  pub fn read<R: io::Read, ConcreteCircuit: Circuit<C::Scalar>>(
    ...
    let fixed_commitments: Vec<_> =
      (0..num_fixed_columns).map(|_| C::read(reader, format)).collect::<Result<_, _>>()?;
    ...
    Ok(Self::from_parts(domain, fixed_commitments, permutation, cs, selectors))
  }
...
impl<C: CurveAffine> VerifyingKey<C> {
    ...
    C::ScalarExt: FromUniformBytes<64>,
  {
    ...
    let mut hasher = Blake2bParams::new().hash_length(64).personal(b"Halo2-Verify-Key").to_state();
  ...
  pub fn fixed_commitments(&self) -> &Vec<C> { &self.fixed_commitments }

  /// Returns `VerifyingKey` of permutation
  pub fn permutation(&self) -> &permutation::VerifyingKey<C> { &self.permutation }

  /// Returns `ConstraintSystem`
  pub fn cs(&self) -> &ConstraintSystem<C::Scalar> { &self.cs }

  /// Returns representative of this `VerifyingKey` in transcripts
  pub fn transcript_repr(&self) -> C::Scalar { self.transcript_repr }
}
...
where C::Scalar: FromUniformBytes<64>
{
  ...
  pub fn get_vk(&self) -> &VerifyingKey<C> { &self.vk }
...
where C::Scalar: SerdePrimeField + FromUniformBytes<64>
{
  ...
  /// Writes a field element into raw bytes in its internal Montgomery representation,
  /// WITHOUT performing the expensive Montgomery reduction.
  /// Does so by first writing the verifying key and then serializing the rest of the data (in the
  /// form of field polynomials)
  pub fn write<W: io::Write>(&self, writer: &mut W, format: SerdeFormat) -> io::Result<()> {
  ...
  /// Does so by reading verification key first, and then deserializing the rest of the file into
  /// the remaining proving key data.
  ///
  /// Reads a curve element from the buffer and parses it according to the `format`:
  /// - `Processed`: Reads a compressed curve element and decompresses it.
  /// Reads a field element in standard form, with endianness specified by the
...
impl<C: CurveAffine> VerifyingKey<C> {
  ...
  pub fn get_domain(&self) -> &EvaluationDomain<C::Scalar> { &self.domain }
}
