...
pub fn circuit_dot_graph<F: Field, ConcreteCircuit: Circuit<F>>(
  ...
  for (parent, child) in graph.edges {
    stmts = stmts.add_edge(Edge::head_node(parent.into(), None).arrow_to_node(child.into(), None))
  }
...
impl<F: Field> Assignment<F> for Graph {
  ...
  fn pop_namespace(&mut self, gadget_name: Option<String>) {
    ...
    let node =
      self.current_namespace.last().expect("pop_namespace should never be called on the root");
