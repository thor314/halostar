...
use crate::{
  ...
  plonk::Error,
  poly::{
    commitment::{CommitmentScheme, MSM},
  ...
  for AccumulatorStrategy<'params, C>
{
  ...
  fn new(params: &'params ParamsIPA<C>) -> Self { AccumulatorStrategy { msm: MSMIPA::new(params) } }
  ...
  #[must_use]
  fn finalize(self) -> bool { self.msm.check() }
}
  ...
  for SingleStrategy<'params, C>
{
  ...
  fn new(params: &'params ParamsIPA<C>) -> Self { SingleStrategy { msm: MSMIPA::new(params) } }
  ...
  /// specific failing proofs, it must re-process the proofs separately.
  #[must_use]
  fn finalize(self) -> bool { unreachable!() }
}
