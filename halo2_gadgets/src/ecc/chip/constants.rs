    ...
    window_table.push(
      (0..H)
        .map(|k| {
          // scalar = (k+2)*(8^w)
          let scalar =
            C::Scalar::from(k as u64 + 2) * C::Scalar::from(H as u64).pow([w as u64, 0, 0, 0]);
...
pub fn compute_lagrange_coeffs<C: CurveAffine>(base: C, num_windows: usize) -> Vec<[C::Base; H]> {
  ...
  window_table
    .iter()
    .map(|window_points| {
      let x_window_points: Vec<_> =
        window_points.iter().map(|point| *point.coordinates().unwrap().x()).collect();
...
pub fn find_zs_and_us<C: CurveAffine>(
  ...
  let find_z_and_us = |window_points: &[C]| {
    ...
    let ys: Vec<_> = window_points.iter().map(|point| *point.coordinates().unwrap().y()).collect();
  ...
  let window_table = compute_window_table(base, num_windows);
  window_table.iter().map(|window_points| find_z_and_us(window_points)).collect()
}
...
pub fn test_lagrange_coeffs<C: CurveAffine>(base: C, num_windows: usize) {
  ...
  fn evaluate<C: CurveAffine>(x: u8, coeffs: &[C::Base]) -> C::Base {
    ...
    coeffs.iter().rev().cloned().reduce(|acc, coeff| acc * x + coeff).unwrap_or(C::Base::ZERO)
  }
...
mod tests {
  ...
  fn zs_and_us() {
    ...
    let (z, u): (Vec<u64>, Vec<[pallas::Base; H]>) =
      find_zs_and_us(base.to_affine(), NUM_WINDOWS).unwrap().into_iter().unzip();
