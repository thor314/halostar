...
pub(super) fn render_cell_layout(
  ...
  // Print the assigned cells, and their region offset or rotation + the column name at which
  // they're assigned to.
  ...
  for (rotation, row) in layout {
    ...
    for ((col, _), &width) in columns.iter().zip(widths.iter()) {
      eprint!("{}|", padded(' ', width, row.get(col).map(|s| s.as_str()).unwrap_or_default()));
    }
  ...
  expr.evaluate(
    ...
    &|query| {
      layout
        .get(&query.rotation.0)
        .and_then(|map| {
          map.get(&(Any::Advice(Advice { phase: query.phase }), query.column_index).into())
        })
        .cloned()
        .unwrap_or_default()
    },
