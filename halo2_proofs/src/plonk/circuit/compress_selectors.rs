...
///
/// This function takes
/// * `selectors`, a vector of `SelectorDescription`s that describe each selector
/// * `max_degree`, the maximum allowed degree of any gate
/// * `allocate_fixed_columns`, a closure that constructs a new fixed column and queries it at
///   Rotation::cur(), returning the expression
///
  ...
  selectors.retain(|selector| {
    if selector.max_degree == 0 {
      ...
      let combination_assignment =
        selector.activations.iter().map(|b| if *b { F::ONE } else { F::ZERO }).collect::<Vec<_>>();
