...
use super::{super::NonIdentityEccPoint, X, Y, Z};
...
impl<const NUM_BITS: usize> Config<NUM_BITS> {
  pub(super) fn configure(
    ...
    let config = Self {
      ...
      z,
      double_and_add: DoubleAndAdd { x_a, x_p, lambda_1, lambda_2 },
      y_p,
    };
  ...
  fn create_gate(&self, meta: &mut ConstraintSystem<pallas::Base>) {
    ...
    let for_loop = |meta: &mut VirtualCells<pallas::Base>, y_a_next: Expression<pallas::Base>| {
  ...
  pub(super) fn double_and_add(
    ...
    x_a.zip(y_a).zip(x_p.zip(y_p)).error_if_known_and(|((x_a, y_a), (x_p, y_p))| {
    ...
    let (mut x_a, mut y_a, mut z) = {
      ...
      // Initialise acc
      let x_a =
        acc.0.copy_advice(|| "starting x_a", region, self.double_and_add.x_a, offset + 1)?;
      let y_a =
        acc.1.copy_advice(|| "starting y_a", region, self.double_and_add.lambda_1, offset)?;
    ...
    for (row, k) in bits.iter().enumerate() {
      ...
      let y_p = y_p.zip(k.as_ref()).map(|(y_p, k)| if !k { -y_p } else { y_p });
      ...
      region.assign_advice(|| "lambda1", self.double_and_add.lambda_1, row + offset, || lambda1)?;

      // x_R = λ1^2 - x_A - x_P
      let x_r =
        lambda1.zip(x_a.value()).zip(x_p).map(|((lambda1, x_a), x_p)| lambda1.square() - x_a - x_p);

      // λ2 = (2(y_A) / (x_A - x_R)) - λ1
      let lambda2 =
        lambda1.zip(y_a).zip(x_a.value()).zip(x_r).map(|(((lambda1, y_a), x_a), x_r)| {
          y_a * pallas::Base::from(2) * (x_a - x_r).invert() - lambda1
        });
      region.assign_advice(|| "lambda2", self.double_and_add.lambda_2, row + offset, || lambda2)?;
      ...
      let x_a_val = x_a_new;
      x_a =
        region.assign_advice(|| "x_a", self.double_and_add.x_a, row + offset + 1, || x_a_val)?;
    }

    // Witness final y_a
    let y_a =
      region.assign_advice(|| "y_a", self.double_and_add.lambda_1, offset + NUM_BITS, || y_a)?;
