...
use halo2_proofs::{
  ...
  plonk::{Advice, Any, Column, ConstraintSystem, Constraints, Error, Expression, Fixed, Selector},
  poly::Rotation,
};
...
impl<F: Field, const WIDTH: usize, const RATE: usize> Pow5Chip<F, WIDTH, RATE> {
  ...
  pub fn construct(config: Pow5Config<F, WIDTH, RATE>) -> Self { Pow5Chip { config } }
}
  ...
  > PoseidonSpongeInstructions<F, S, D, WIDTH, RATE> for Pow5Chip<F, WIDTH, RATE>
{
    ...
    layouter.assign_region(
      ...
      |mut region| {
        ...
        let load_state_word = |i: usize| {
          initial_state[i]
            .0
            .copy_advice(|| format!("load state_{}", i), &mut region, config.state[i], 0)
            .map(StateWord)
        };
        let initial_state: Result<Vec<_>, Error> = (0..WIDTH).map(load_state_word).collect();
        ...
        let load_input_word = |i: usize| {
          ...
          constraint_var
            .copy_advice(|| format!("load input_{}", i), &mut region, config.state[i], 1)
            .map(StateWord)
        };
  ...
  fn get_output(state: &State<Self::Word, WIDTH>) -> Squeezing<Self::Word, RATE> {
    Squeezing(
      state[..RATE].iter().map(|word| Some(word.clone())).collect::<Vec<_>>().try_into().unwrap(),
    )
  }
}
...
impl<F: Field> From<StateWord<F>> for AssignedCell<F, F> {
  fn from(state_word: StateWord<F>) -> AssignedCell<F, F> { state_word.0 }
}

impl<F: Field> From<AssignedCell<F, F>> for StateWord<F> {
  fn from(cell_value: AssignedCell<F, F>) -> StateWord<F> { StateWord(cell_value) }
}

impl<F: Field> Var<F> for StateWord<F> {
  fn cell(&self) -> Cell { self.0.cell() }

  fn value(&self) -> Value<F> { self.0.value().cloned() }
}
  ...
  fn full_round<const RATE: usize>(
    ...
    Self::round(region, config, round, offset, config.s_full, |_| {
      let q = self
        ...
        .iter()
        .enumerate()
        .map(|(idx, word)| word.0.value().map(|v| *v + config.round_constants[round][idx]));
      let r: Value<Vec<F>> = q.map(|q| q.map(|q| q.pow(config.alpha))).collect();
      let m = &config.m_reg;
      let state = m.iter().map(|m_i| {
        r.as_ref().map(|r| r.iter().enumerate().fold(F::ZERO, |acc, (j, r_j)| acc + m_i[j] * r_j))
      });
  ...
  fn partial_round<const RATE: usize>(
    ...
    Self::round(region, config, round, offset, config.s_partial, |region| {
      ...
      let r: Value<Vec<_>> = p.map(|p| {
        ...
        let r_i =
          p[1..].iter().enumerate().map(|(i, p_i)| *p_i + config.round_constants[round][i + 1]);
      ...
      let p_mid: Value<Vec<_>> = m
        ...
        .map(|m_i| {
          r.as_ref()
            .map(|r| m_i.iter().zip(r.iter()).fold(F::ZERO, |acc, (m_ij, r_j)| acc + *m_ij * r_j))
        })
        .collect();
      ...
      let r_mid: Value<Vec<_>> = p_mid.map(|p| {
        ...
        let r_i =
          p[1..].iter().enumerate().map(|(i, p_i)| *p_i + config.round_constants[round + 1][i + 1]);
        std::iter::empty().chain(Some(r_0)).chain(r_i).collect()
      });
...
mod tests {
    ...
    for PermuteCircuit<S, WIDTH, RATE>
  {
    ...
    fn without_witnesses(&self) -> Self { PermuteCircuit::<S, WIDTH, RATE>(PhantomData) }
    ...
    fn synthesize(
      ...
      let initial_state = layouter.assign_region(
        ...
        |mut region| {
          let state_word = |i: usize| {
            ...
            let var =
              region.assign_advice(|| format!("load state_{}", i), config.state[i], 0, || value)?;
            Ok(StateWord(var))
          };
      ...
      let mut expected_final_state =
        (0..WIDTH).map(|idx| Fp::from(idx as u64)).collect::<Vec<_>>().try_into().unwrap();
      let (round_constants, mds, _) = S::constants();
      poseidon::permute::<_, S, WIDTH, RATE>(&mut expected_final_state, &mds, &round_constants);
    ...
    for HashCircuit<S, WIDTH, RATE, L>
  {
    ...
    fn without_witnesses(&self) -> Self {
      Self { message: Value::unknown(), output: Value::unknown(), _spec: PhantomData }
    }
    ...
    fn synthesize(
      ...
      let message = layouter.assign_region(
        ...
        |mut region| {
          let message_word = |i: usize| {
            ...
            region.assign_advice(|| format!("load message_{}", i), config.state[i], 0, || value)
          };
      ...
      let hasher =
        Hash::<_, _, S, ConstantLength<L>, WIDTH, RATE>::init(chip, layouter.namespace(|| "init"))?;
  ...
  fn print_poseidon_chip() {
    ...
    let root = root.titled("Poseidon Chip Layout", ("sans-serif", 60)).unwrap();
    ...
    halo2_proofs::dev::CircuitLayout::default().render(6, &circuit, &root).unwrap();
  }
}
